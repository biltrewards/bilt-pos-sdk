...
class DisplayPayloadHelperTest {
  ...
  void encodeAndDecodeShouldRoundTripCorrectly() throws JAXBException {
    ...
    assertEquals(
        "Running shoes", decoded.getReceipt().getLineItems().getLineItem().get(0).getDescription());
  }
  ...
  void inputPayloadToBase64ShouldProduceDecodableXml() throws JAXBException {
    ...
    String base64 = DisplayPayloadHelper.toBase64(original);
    String xml =
        new String(Base64.getDecoder().decode(base64), java.nio.charset.StandardCharsets.UTF_8);
  ...
  void qrCodeWithHeaderAndCtaShouldIncludeBoth() throws JAXBException {
    DisplayPayload payload =
        DisplayPayloadHelper.qrCode("qr.xslt", "https://example.com", "Scan me", "Scan now");
  ...
  void productItemShouldCreateCompleteLineItem() {
    LineItemType item =
        DisplayPayloadHelper.productItem(
            "Widget", new BigDecimal("2"), "$", new BigDecimal("10.00"), new BigDecimal("20.00"));
  ...
  void imagePayloadShouldSerializeValueAsBase64ElementText() throws JAXBException {
    ...
    assertTrue(
        xml.contains(">" + expectedBase64 + "</image>"),
  ...
  void imagePayloadShouldRoundTrip() throws JAXBException {
    ...
    DisplayPayload parsed =
        DisplayPayloadHelper.fromBase64(DisplayPayloadHelper.toBase64(original));
