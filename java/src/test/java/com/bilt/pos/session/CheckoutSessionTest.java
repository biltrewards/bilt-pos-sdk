...
class CheckoutSessionTest {
  ...
  /**
   * A dispatcher answering the session start/end Admin exchanges out of band, so tests enqueue
   * their responses in wire order without the bracket eating them.
   */
  ...
  void transportFailureIsMappedToNetworkError() throws Exception {
    ...
    SessionException e =
        assertThrows(SessionException.class, () -> session.terminal().diagnose().get());
  ...
  void reconcileSendsSaleReconciliationAndMapsTotals() throws Exception {
    ...
    assertEquals(
        "SaleReconciliation", sent.getReconciliationRequest().getReconciliationType().toValue());
  }
  ...
  void transactionStatusFoundRepeatsOriginalPaymentResponse() throws Exception {
    ...
    assertEquals(
        "SVC0084200", sent.getTransactionStatusRequest().getMessageReference().getServiceID());
    assertEquals(
        "POS-LANE-3", sent.getTransactionStatusRequest().getMessageReference().getSaleID());
  }
  ...
  void updateDisplayGoesToExternalDisplayWhenConfigured() throws Exception {
    try (MockWebServer displayServer = new MockWebServer()) {
      ...
      assertEquals(
          "CustomerDisplay", sent.getDisplayRequest().getDisplayOutput()[0].getDevice().toValue());
      assertNotNull(
