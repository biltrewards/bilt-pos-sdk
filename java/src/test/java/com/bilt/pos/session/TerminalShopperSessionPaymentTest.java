...
class TerminalShopperSessionPaymentTest {
  ...
  void unexpectedFlowExceptionIsNotMaskedAsSuccess() {
    ...
    IllegalStateException first = assertThrows(IllegalStateException.class, flow::executeSync);
  ...
  void throwingSuccessHandlerStaysLoudOnLaterAccessors() {
    SettlementFlow flow =
        new SettlementFlow(f -> SettlementResult.builder().success(true).build())
            .onSuccess(
                result -> {
                  throw new NullPointerException("register bug");
                });

    NullPointerException first = assertThrows(NullPointerException.class, flow::executeSync);
  ...
  void executeDeliversPreOrchestrationFailuresToOnError() {
    ...
    assertEquals(2, server.getRequestCount(), "only the session start and end may hit the wire");
  }
  ...
  void executeDeliversAFailedStandingDrainToOnError() throws Exception {
    ...
    assertTrue(seen.get().getMessage().contains("retry did not start"), seen.get().getMessage());
  }
  ...
  void payIsInertUntilExecuted() throws Exception {
    ...
    SettlementFlow flow =
        session.settle().onSuccess(r -> {}).onError(e -> SettlementRecovery.abort());
  ...
  void payRequiresItems() {
    ...
    assertEquals(
        1, server.getRequestCount(), "nothing beyond the session start may reach the wire");
  }
  ...
  void settlementAllowsAZeroTotalBasket() {
    ...
    session
        ...
        .mutate(
            m ->
                m.removeItemBySku("SKU-1")
                    .addItem(
                        BasketItem.sale("SKU-FREE", "Comped Item", 1, new BigDecimal("0.00"))));
  ...
  void storeLocationIsSentAsTotalsGroupId() throws Exception {
    ...
    assertEquals(
        "STR-0142",
        sent.getPaymentRequest().getSaleData().getSaleTerminalData().getTotalsGroupID(),
  ...
  void guestCheckoutIsCardOnly() throws Exception {
    ...
    assertEquals(
        ...
        requests
            ...
            .getRequestedAmount());
    assertEquals(
        1, requests.get(0).getPaymentRequest().getPaymentTransaction().getSaleItem().length);
  }
  ...
  void completedVoidStatusIsOwnedByTheTerminal() throws Exception {
    ...
    server.enqueue(new MockResponse().setBody(PAYMENT_DECLINED));
    assertThrows(SessionException.class, () -> session.refund(new BigDecimal("10.00")).get());
  ...
  void settleChargesBasketLevelTaxTotalOverride() throws Exception {
    ...
    assertEquals(0, new BigDecimal("8.88").compareTo(result.getFinalBasket().getTaxTotal()));

    SaleToPOIRequest sent = nextRequest();
    assertEquals(
        108.88,
        sent.getPaymentRequest().getPaymentTransaction().getAmountsReq().getRequestedAmount(),
  ...
  void earnedRewardsFromTheAwardResponseReachTheResult() throws Exception {
    ...
    assertEquals(
        1, result.getEarnedRewards().size(), "rewards earned by the award must reach the result");
  ...
  void cashbackIsIncludedInTheRequestedAmount() throws Exception {
    ...
    SettlementResult result =
        session.settle(SettlementOptions.builder().cashback(new BigDecimal("20.00")).build()).get();
    ...
    assertEquals(
        20.00,
        sent.getPaymentRequest().getPaymentTransaction().getAmountsReq().getCashBackAmount());
  }
  ...
  void cashbackUnderAuthorizationFailsAndReverses() throws Exception {
    ...
    SessionException failure =
        assertThrows(
            ...
            () ->
                session
                    .settle(SettlementOptions.builder().cashback(new BigDecimal("20.00")).build())
                    .get());
    ...
    assertEquals(
        "Reversal",
        requests.get(1).getMessageHeader().getMessageCategory().toValue(),
  ...
  void fullFlowThreadsHandlerTotalsThroughSteps() throws Exception {
        ...
        session
            ...
            .onRebatesRedeemed(
                rebates -> {
                  ...
                  assertEquals(
                      0, new BigDecimal("10.00").compareTo(rebates.getTotalRebateAmount()));
                  assertEquals("Gold: $10 off", rebates.getRebates().get(0).getLabel());
                  assertEquals(
                      0,
                      new BigDecimal("90.00")
                          .compareTo(rebates.getUpdatedBasket().getItem("1").getAdjustedTotal()));
    ...
    assertEquals(0, new BigDecimal("10.00").compareTo(result.getFinalBasket().getRebateTotal()));
    assertEquals(
        0, new BigDecimal("84.50").compareTo(result.getFinalBasket().getCardPaymentTotal()));
    ...
    assertEquals(
        ...
        requests
            ...
            .toValue());
    assertEquals(
        100.00, requests.get(0).getLoyaltyRequest().getLoyaltyTransaction().getTotalAmount());
    assertEquals(
        "Redemption",
        requests
            .get(1)
    ...
    assertEquals(
        90.00,
        requests.get(1).getLoyaltyRequest().getLoyaltyTransaction().getTotalAmount(),
        "loyalty TotalAmount is the item sum (rebate-adjusted), not the running total");
    assertNotNull(
        requests.get(1).getLoyaltyRequest().getSaleData().getSaleToPOIData(),
    ...
    assertEquals(
        "USD",
        requests.get(1).getLoyaltyRequest().getLoyaltyData()[0].getLoyaltyAmount().getCurrency());
    assertNull(
        requests.get(0).getLoyaltyRequest().getLoyaltyData()[0].getLoyaltyAmount(),
    ...
    assertEquals(
        ...
        requests
            ...
            .getSaleItem()[0]
            .getItemAmount(),
        "card step sends rebate-adjusted items");
    assertEquals(
        "Award",
        requests
            .get(3)
    ...
    for (int i : new int[] {0, 1, 3}) {
      double itemSum = 0;
      for (com.bilt.pos.nexo.model.SaleItem item :
          requests.get(i).getLoyaltyRequest().getLoyaltyTransaction().getSaleItem()) {
        itemSum += item.getItemAmount();
      }
      assertEquals(
          itemSum,
          requests.get(i).getLoyaltyRequest().getLoyaltyTransaction().getTotalAmount(),
          0.001,
        ...
        session
            ...
            .onRebatesRedeemed(
                rebates -> {
                  ...
                  assertNull(
                      rebates.getRebates().get(0).getItemId(),
                      "an unattributed rebate surfaces as a cart-level entry");
                  assertEquals(
                      0,
                      new BigDecimal("10.00").compareTo(rebates.getRebates().get(0).getAmount()));
                  assertEquals("Fall Promo", rebates.getRebates().get(0).getLabel());
                  // 10.00 prorated by weight: 50→6.67, 25 (last) absorbs 3.33
                  assertEquals(
                      0,
                      new BigDecimal("43.33")
                          .compareTo(rebates.getUpdatedBasket().getItem("1").getAdjustedTotal()));
                  assertEquals(
                      0,
                      new BigDecimal("21.67")
                          .compareTo(rebates.getUpdatedBasket().getItem("2").getAdjustedTotal()));
                  assertEquals(
                      "Fall Promo", rebates.getUpdatedBasket().getItem("1").getRebateLabel());
  ...
  void throwingMutationDoesNotResumeAFailedSession() throws Exception {
    ...
    assertThrows(
        IllegalArgumentException.class, () -> session.basket().removeItemBySku("NO-SUCH-SKU"));
  ...
  void incompleteRollbackBlocksRetryAndSurfacesUnreversedMovements() throws Exception {
    ...
    assertTrue(
        failure.getError().getMessage().contains("REBATE"),
    ...
    List<SaleToPOIRequest> requests = drainRequests();
    assertEquals(5, requests.size(), "no retry may run on top of an incomplete unwind");
    assertEquals(
        "RebateRefund",
        requests
            .get(4)
  ...
  void abortWithIncompleteRollbackSettlesFailedSoTheUnwindCanFinish() throws Exception {
    ...
    assertTrue(
        failure.getError().getMessage().contains("POINT_REDEMPTION"),
        failure.getError().getMessage());
  ...
  void emptyBasketCannotPayFromAFailedSessionWithStandingMovements() throws Exception {
    ...
    assertEquals(SessionErrorCode.INVALID_STATE, failure.getError().getCode());
    assertEquals(requestsBefore, server.getRequestCount(), "nothing may reach the wire");
  ...
  void abortWithNoOperationInFlightLeavesStandingMovementsToTheVoid() throws Exception {
    ...
    assertEquals(
        requestsBefore, server.getRequestCount(), "an abort with nothing in flight sends nothing");
  ...
  void retriedPayRefusesToStartOverAStillStandingMovement() throws Exception {
    ...
    SessionException failure = assertThrows(SessionException.class, () -> session.settle().get());
    assertTrue(
        failure.getError().getMessage().contains("retry did not start"),
        failure.getError().getMessage());
    assertEquals(1, drainRequests().size(), "no new charge may start over a standing movement");
  ...
  void pointRedemptionIsSkippedWhenRebatesCoverTheTotal() throws Exception {
    ...
    List<SaleToPOIRequest> requests = drainRequests();
    assertEquals(
        2, requests.size(), "rebate and award only — a zeroed total must not trigger a redemption");
    assertEquals(
        "Rebate",
        requests
            .get(0)
  ...
  void cartLevelProrationNeverProducesANegativeLineRebate() throws Exception {
    ...
    for (int i = 1; i <= 7; i++) {
      session
          .basket()
          .addItem(BasketItem.sale("SKU-" + i, "Item " + i, 1, new BigDecimal("10.00")));
    }
            ...
            .onRebatesRedeemed(
                rebates -> {
                  ...
                  for (var line : rebates.getUpdatedBasket().getItems()) {
                    assertTrue(
                        line.getRebateAmount().signum() >= 0,
                        line.getItemId() + " got a negative rebate: " + line.getRebateAmount());
                    assertTrue(
                        line.getAdjustedTotal().compareTo(line.getOriginalTotal()) <= 0,
                        "a rebate must never raise a line's total");
                    lineSum = lineSum.add(line.getRebateAmount());
                  }
                  assertEquals(
                      0,
                      new BigDecimal("0.05").compareTo(lineSum),
  ...
  void voidOfLoyaltyPaidCheckoutRefundsRedemptionAndRebate() throws Exception {
    ...
    assertEquals("Reversal", requests.get(0).getMessageHeader().getMessageCategory().toValue());
    assertEquals(
        "RedemptionRefund",
        requests
            .get(1)
  ...
  void failedLoyaltyRefundDoesNotFailThePaidVoid() throws Exception {
    ...
    assertTrue(
        session.voidTransaction().get().isSuccess(),
        "the tender was reversed; a loyalty refund failure is best-effort (SAF)");
    assertEquals(
        4, drainRequests().size(), "the remaining loyalty refunds still run after one fails");
  }
  ...
  void rewardOnlyCheckoutVoidsViaLoyaltyRefunds() throws Exception {
    ...
    assertEquals(
        ...
        requests
            ...
            .getTransactionID());
    assertNull(
        requests.get(0).getLoyaltyRequest().getSaleData().getSaleToPOIData(),
        "per the reversal contract the original transaction reference suffices");
    assertEquals(
        "98234",
        requests
  ...
  void rewardOnlyVoidRetryResumesAtRebateLeg() throws Exception {
    ...
    SessionException failure =
        assertThrows(SessionException.class, () -> session.voidTransaction().get());
    assertTrue(
        failure.getError().getMessage().contains("POI-RD-1"),
  ...
  void originalSaleRecordVoidRequiresAReference() {
    ...
    assertEquals(SessionErrorCode.INVALID_STATE, failure.getError().getCode());
    assertTrue(
        failure.getError().getMessage().contains("at least one original transaction reference"));
    assertEquals(1, server.getRequestCount(), "only the session start may hit the wire");
  }
  ...
  void originalSaleRecordVoidRejectsTheMostRecentSessionPayment() throws Exception {
    ...
    SessionException referencedVoid =
        assertThrows(
            SessionException.class,
    ...
    assertTrue(referencedVoid.getError().getMessage().contains("parameterless voidTransaction()"));
    assertEquals(
        0, drainRequests().size(), "a referenced void must not bypass the latest payment's guards");
  ...
  void originalSaleRecordVoidCanTargetAnOlderSettlementInTheSameSession() throws Exception {
    ...
    server.enqueue(new MockResponse().setBody(REVERSAL_OK));
    assertTrue(session.voidTransaction(olderSale.toOriginalSaleRecord(null)).get().isSuccess());
  ...
  void originalSaleRecordVoidCanRunAlongsideAnUnsettledBasket() throws Exception {
    ...
    assertEquals(
        1,
        session.basket().snapshot().getItemCount(),
  ...
  void originalSaleRecordRedemptionOnlyVoidOmitsLoyaltyDataWithoutMember() throws Exception {
    ...
    assertEquals(
        ...
        redemptionRefund
            ...
            .getTransactionID());
    assertNull(
        redemptionRefund.getLoyaltyRequest().getLoyaltyData(),
  ...
  void originalSaleRecordRebateOnlyVoidIncludesMemberWhenKnown() throws Exception {
    ...
    assertEquals(
        "98234",
        rebateRefund.getLoyaltyRequest().getLoyaltyData()[0].getLoyaltyAccountID().getLoyaltyID());
  }
  ...
  void originalSaleRecordVoidRetryResumesAfterPartialFailure() throws Exception {
    ...
    SessionException failure =
        assertThrows(SessionException.class, () -> session.voidTransaction(originalSale).get());
    assertTrue(failure.getError().getMessage().contains(PRIOR_CARD_POI_TXN + " was reversed"));
    assertTrue(failure.getError().getMessage().contains(PRIOR_STORED_VALUE_POI_TXN + " was not"));
    ...
    SessionException endRefused = assertThrows(SessionException.class, () -> session.end().get());
    assertEquals(SessionErrorCode.INVALID_STATE, endRefused.getError().getCode());
    assertTrue(endRefused.getError().getMessage().contains("voidTransaction(OriginalSaleRecord)"));
    assertEquals(0, drainRequests().size(), "end must not discard prior-sale void resume progress");

    assertDoesNotThrow(session::close, "best-effort close must leave the guarded session open");
    assertEquals(0, drainRequests().size(), "close must inherit end's prior-sale progress guard");

    SessionException wrongSale =
        assertThrows(
            SessionException.class,
            () ->
                session
  ...
  void finalBasketReflectsHandlerRecalculatedTax() throws Exception {
    ...
    assertEquals(
        0,
        new BigDecimal("7.20").compareTo(result.getFinalBasket().getTaxTotal()),
        "the recalculated tax must reach the final basket");
    assertEquals(0, new BigDecimal("107.20").compareTo(result.getFinalBasket().getGrandTotal()));
  ...
  void beforeStepControlsSaleTransactionIds() throws Exception {
    ...
    assertEquals(
        ...
        requests
            ...
            .getTransactionID());
    assertEquals(
        "TXN-" + SettlementStep.CARD_CHARGE,
        requests
            .get(2)
            .getPaymentRequest()
            .getSaleData()
        ...
        session
            ...
            .onPointsRedeemed(
                points -> {
                  ...
                  assertEquals(
                      0,
                      BigDecimal.ZERO.compareTo(points.getMonetaryValue()),
                      "a point count must not become a dollar discount");
                  assertEquals(0, points.getPreviousTotal().compareTo(points.getSuggestedTotal()));
                  return points.getSuggestedTotal();
                })
            .get();
  ...
  void skipFailedRebateContinuesWithPayment() throws Exception {
    ...
    assertEquals(4, requests.size());
    assertNotNull(
        requests.get(2).getPaymentRequest(),
  ...
  void skippedCommittedRebateDoesNotLeaveAReversedReference() throws Exception {
    ...
    SettlementResult result =
        session
            .settle(SettlementOptions.builder().disablePoints(true).disableAward(true).build())
            .onMovement(
                movement -> {
    ...
    assertNull(
        result.toOriginalSaleRecord("98234").getRebatePoiTransactionId(),
  ...
  void onErrorRunsBeforeAbortUnwindsCommittedSteps() throws Exception {
    ...
    server.enqueue(new MockResponse().setBody(LOYALTY_REFUND_OK));
    server.enqueue(new MockResponse().setBody(LOYALTY_REFUND_OK));
    AtomicReference<List<SaleToPOIRequest>> requestsSeenByHandler = new AtomicReference<>();
    ...
    assertTrue(
        requestsSeenByHandler.get().stream()
            .noneMatch(
                request ->
  ...
  void failedCardCanBeReplacedByExternalCashPayment() throws Exception {
    ...
    assertEquals(0, BigDecimal.ZERO.compareTo(result.getCardAmountCharged()));
    assertEquals(0, new BigDecimal("100.00").compareTo(result.getExternalPaymentAmount()));
    assertEquals(
        0, new BigDecimal("100.00").compareTo(result.getFinalBasket().getExternalPaymentTotal()));
    ...
    assertEquals(
        1, drainRequests().size(), "register-managed cash must send no second terminal payment");
  }
  ...
  void externalPaymentMustEqualTheOutstandingCardTender() throws Exception {
        ...
        assertThrows(
            ...
            () ->
                session
                    ...
                    .onError(
                        error ->
                            error.getStep() == SettlementStep.CARD_CHARGE
                                ? SettlementRecovery.external(
                                    ExternalPayment.cash(new BigDecimal("64.00"), "DRAWER-42"))
    ...
    assertTrue(
        failure
            .getError()
            .getMessage()
            .contains("external payment 64.00 must equal the outstanding 65.00"));
    List<SaleToPOIRequest> requests = drainRequests();
    assertEquals(2, requests.size());
    assertTrue(
        requests.stream().noneMatch(request -> request.getReversalRequest() != null),
        "invalid recovery must not silently unwind the stored-value tender");

    SessionException endFailure = assertThrows(SessionException.class, () -> session.end().get());
    assertTrue(
        endFailure.getError().getMessage().contains("rollback is incomplete"),
  ...
  void externalPaymentCannotReplaceACardTenderWithCashback() throws Exception {
                ...
                session
                    ...
                    .onError(
                        error ->
                            error.getStep() == SettlementStep.CARD_CHARGE
                                ? SettlementRecovery.external(
                                    ExternalPayment.cash(error.getAmountDue(), "DRAWER-42"))
                                : SettlementRecovery.abort())
                    .get());
    ...
    assertTrue(
        failure
            .getError()
            .getMessage()
            .contains("cannot replace a card tender that includes cashback"));
    List<SaleToPOIRequest> requests = drainRequests();
    assertEquals(2, requests.size());
    assertTrue(
        requests.stream().noneMatch(request -> request.getReversalRequest() != null),
  ...
  @Test
  void abandonTransfersCommittedMovementsAndLeavesBasketImmediatelyReusable() throws Exception {
    identifyMember();
    addHundredDollarItem();
    server.enqueue(new MockResponse().setBody(REBATE_OK));
    server.enqueue(new MockResponse().setBody(REDEEM_OK));
    ...
    assertEquals(SettlementStep.CARD_CHARGE, callbackRecord.get().getFailure().getStep());
    assertEquals(2, callbackRecord.get().getCommittedMovements().size());
    assertEquals(1, session.basket().snapshot().getItemCount());
    List<SaleToPOIRequest> firstAttempt = drainRequests();
    assertEquals(3, firstAttempt.size());
    assertTrue(firstAttempt.stream().noneMatch(request -> request.getReversalRequest() != null));
  ...
  void indeterminateCardCanBeAbandonedAfterAutomaticStatusRecovery() throws Exception {
    ...
    server.enqueue(new MockResponse().setBody(TRANSACTION_STATUS_WITHOUT_REPEATED_RESPONSE));
    AtomicInteger errorCalls = new AtomicInteger();
                    ...
                    .onError(
                        context -> {
                          ...
                          assertTrue(
                              context
                                  .getMessage()
                                  .contains("could not resolve CARD_CHARGE request"));
    ...
    assertEquals(1, errorCalls.get());
    List<SaleToPOIRequest> requests = drainRequests();
    assertEquals(2, requests.size(), "automatic status recovery must precede the abandon decision");
  ...
  void indeterminateCardSuccessIsRecoveredBeforeOnError() throws Exception {
    ...
    server.enqueue(
        new MockResponse().setBody(transactionStatusPaymentOk("POI-PAY-STATUS", 100.00)));
    AtomicInteger errorCalls = new AtomicInteger();
    ...
    assertEquals(
        0, errorCalls.get(), "a recovered successful request must not be reported as a failure");
    List<SaleToPOIRequest> requests = drainRequests();
    assertEquals(2, requests.size(), "status recovery must avoid a second payment request");
    assertNotNull(requests.get(0).getPaymentRequest());
    assertNotNull(requests.get(1).getTransactionStatusRequest());
    assertEquals(
        requests.get(0).getMessageHeader().getServiceID(),
        requests.get(1).getTransactionStatusRequest().getMessageReference().getServiceID());
  }

  @Test
  void transactionStatusNotFoundReportsADefinitiveFailureBeforeRetry() throws Exception {
    session =
        start(
            sessionBuilder()
                .client(
  ...
  @Test
  void recoveredCardFailureIsReportedAsDefinitiveBeforeExternalPayment() throws Exception {
    session =
        start(
            sessionBuilder()
                .client(
    ...
    server.enqueue(
        new MockResponse()
            .setBodyDelay(500, TimeUnit.MILLISECONDS)
            .setBody(paymentOk("POI-PAY-UNKNOWN", 100.00)));
    server.enqueue(new MockResponse().setBody(TRANSACTION_STATUS_PAYMENT_DECLINED));
    AtomicReference<SettlementFailure> reported = new AtomicReference<>();
    ...
    SettlementResult result =
        session
            ...
            .onError(
                failure -> {
                  reported.set(failure);
                  return SettlementRecovery.external(
                      ExternalPayment.cash(failure.getAmountDue(), "DRAWER-42"));
                })
            .get();
    ...
    List<SaleToPOIRequest> requests = drainRequests();
    assertEquals(
        2, requests.size(), "the external tender must not send another terminal payment request");
    assertNotNull(requests.get(0).getPaymentRequest());
    assertNotNull(requests.get(1).getTransactionStatusRequest());
  ...
  @Test
  void retryAfterUnresolvedStatusChecksStatusAgainWithoutResendingPayment() throws Exception {
    session =
        start(
            sessionBuilder()
                .client(
  ...
  void externalPaymentCannotReplaceAnUnresolvedCardRequest() throws Exception {
                ...
                session
                    ...
                    .onError(
                        context -> {
                          ...
                          assertTrue(context.isIndeterminate());
                          return SettlementRecovery.external(
                              ExternalPayment.cash(context.getAmountDue(), "DRAWER-42"));
                        })
                    .get());
    ...
    assertTrue(
        failure
            .getError()
            .getMessage()
            .contains("cannot proceed while TransactionStatus has not resolved"));
    List<SaleToPOIRequest> requests = drainRequests();
    assertEquals(2, requests.size());
    assertNotNull(requests.get(0).getPaymentRequest());
    assertNotNull(requests.get(1).getTransactionStatusRequest());
  ...
  @Test
  void recoveredCardChargeCompletesBeforeRequestingExternalPayment() throws Exception {
    session =
        start(
            sessionBuilder()
                .client(
    ...
    assertEquals(
        0, errorCalls.get(), "the register must not be asked to collect an external tender");
    assertEquals(0, externalCalls.get(), "no external tender must be recorded");
    assertEquals(0, new BigDecimal("100.00").compareTo(result.getCardAmountCharged()));
    ...
    List<SaleToPOIRequest> requests = drainRequests();
    assertEquals(
        2, requests.size(), "status recovery must not record or send another payment request");
    assertNotNull(requests.get(0).getPaymentRequest());
    assertNotNull(requests.get(1).getTransactionStatusRequest());
  }
  ...
  void indeterminateCardRecoveryPollsWhileTransactionIsInProgress() throws Exception {
    ...
    List<SaleToPOIRequest> requests = drainRequests();
    assertEquals(4, requests.size(), "the payment and three bounded status checks must be sent");
    assertNotNull(requests.get(0).getPaymentRequest());
    ...
    for (int i = 1; i < requests.size(); i++) {
      ...
      assertEquals(
          requests.get(0).getMessageHeader().getServiceID(),
          status.getTransactionStatusRequest().getMessageReference().getServiceID());
      statusServiceIds.add(status.getMessageHeader().getServiceID());
    }
    assertEquals(3, statusServiceIds.size(), "each status poll must have a fresh ServiceID");
  }

  @Test
  void indeterminateCardRecoveryStopsAfterBoundedInProgressResponses() throws Exception {
    session =
        start(
            sessionBuilder()
                .client(
    ...
    assertTrue(failure.getError().getMessage().contains("still processing Payment request"));
    assertTrue(failure.getError().getMessage().contains("after 5 TransactionStatus attempts"));
    assertEquals(6, drainRequests().size(), "recovery must stop after five status checks");
  }
  ...
  void indeterminateCardDoesNotResendWhenStatusCannotResolve() throws Exception {
    ...
    List<SaleToPOIRequest> requests = drainRequests();
    assertEquals(
        2, requests.size(), "an inconclusive status response must never cause a duplicate payment");
    assertNotNull(requests.get(0).getPaymentRequest());
    assertNotNull(requests.get(1).getTransactionStatusRequest());
  }
  ...
  void basketCanBeAdjustedAfterAFailedPayment() throws Exception {
    ...
    session
        .basket()
        .addItem(BasketItem.sale("SKU-2", "Expensive Item", 1, new BigDecimal("50.00")));
    server.enqueue(new MockResponse().setBody(PAYMENT_DECLINED));
  ...
  @Test
  void abandoningUnderAuthorizedCardReportsTheStandingMovementHonestly() throws Exception {
    addHundredDollarItem();
    server.enqueue(
        new MockResponse()
            .setBody(
    ...
    SessionException abandoned =
        assertThrows(
            SessionException.class,
            () -> session.settle().onError(ignored -> SettlementRecovery.abandon()).get());
    ...
    assertEquals(
        "the card payment authorized only 60.00 of the requested 100.00", failure.getMessage());
    assertEquals(1, failure.getCommittedMovements().size());
    assertEquals(SettlementStep.CARD_CHARGE, failure.getCommittedMovements().get(0).getStep());
    assertEquals("POI-PAY-1", failure.getCommittedMovements().get(0).getPoiTransactionId());
    assertEquals(
        1, drainRequests().size(), "abandon must leave the partial authorization standing");
  }
  ...
  void retryResetsOnlyAPartiallyCommittedFailedCardStep() throws Exception {
    ...
    SettlementResult result = session.settle().onError(error -> SettlementRecovery.retry()).get();

    assertEquals("POI-PAY-2", result.getPoiTransactionId());
    assertEquals(
        1,
        result.getMovements().size(),
  ...
  void throwingHandlerUnwindsCommittedStepsAndFailsTheSession() throws Exception {
    ...
    assertTrue(
        session
            .settle(SettlementOptions.builder().disableRebates(true).disablePoints(true).build())
            .get()
            .isSuccess());
  }
  ...
  void throwingAwardHandlerConsultsOnErrorBeforeUnwinding() throws Exception {
    ...
    assertEquals(
        2,
        seen.get().getCommittedMovements().size(),
  ...
  @Test
  void throwingExternalPaymentHandlerConsultsOnErrorAndUnwindsEarlierTender() throws Exception {
    addHundredDollarItem();
    session.setStoredValueCard("GC-1234-5678");
    server.enqueue(new MockResponse().setBody(paymentOk("POI-GC-1", 35.00)));
    server.enqueue(new MockResponse().setBody(PAYMENT_DECLINED));
  ...
  @Test
  void throwingExternalPaymentHandlerCanRetryWithoutDuplicatingTheMovement() throws Exception {
    addHundredDollarItem();
    server.enqueue(new MockResponse().setBody(PAYMENT_DECLINED));
    ...
    assertEquals(List.of(SettlementStep.CARD_CHARGE, SettlementStep.EXTERNAL_PAYMENT), failedSteps);
    assertEquals(2, deliveries.get());
    assertEquals(
        1,
        result.getMovements().size(),
        "retry must discard the failed external movement checkpoint");
    assertEquals(SettlementStep.EXTERNAL_PAYMENT, result.getMovements().get(0).getStep());
    assertEquals(0, new BigDecimal("100.00").compareTo(result.getExternalPaymentAmount()));
    assertEquals(
        1, drainRequests().size(), "retrying register-managed payment sends no terminal request");
  }

  @Test
  void throwingExternalPaymentBeforeStepConsultsOnErrorAndUnwindsEarlierTender() throws Exception {
    addHundredDollarItem();
    session.setStoredValueCard("GC-1234-5678");
    server.enqueue(new MockResponse().setBody(paymentOk("POI-GC-1", 35.00)));
    server.enqueue(new MockResponse().setBody(PAYMENT_DECLINED));
  ...
  void disableAwardSkipsTheAwardStepButKeepsRedemptions() throws Exception {
    ...
    SettlementResult result =
        session.settle(SettlementOptions.builder().disableAward(true).build()).get();
  ...
  void refundWithSkippedTenderLeavesThePaymentVoidable() throws Exception {
    ...
    assertTrue(
        session
            .refund(new BigDecimal("10.00"))
            .onError(
                (step, error) ->
                    step == ReversalStep.CARD ? ReversalDecision.SKIP : ReversalDecision.ABORT)
            .get()
            .isSuccess());
    drainRequests();
    ...
    List<SaleToPOIRequest> requests = drainRequests();
    assertEquals(
        1, requests.size(), "one reversal, no second AwardRefund for the already-reversed award");
    assertEquals(
        "POI-PAY-1",
        requests
            .get(0)
  ...
  void abortedAwardReversalAfterTheTenderRefundStillRaisesTheVoidGuard() throws Exception {
    ...
    assertTrue(
        e.getError().getMessage().contains("refund"),
        "the error must steer the register to further refunds: " + e.getError().getMessage());
  ...
  void failedVoidAfterPaymentDoesNotAllowRePayment() throws Exception {
    ...
    assertEquals(
        ...
        assertThrows(SessionException.class, () -> session.settle().get()).getError().getCode());
  ...
  void voidOfSplitTenderReversesBothLegs() throws Exception {
    ...
    assertEquals(
        0,
        new BigDecimal("100.00").compareTo(voided.getReversedAmount()),
        "both legs must be reversed");

    List<SaleToPOIRequest> requests = drainRequests();
    assertEquals(
        2, requests.size(), "a guest checkout has no loyalty movements — two reversals only");
    assertEquals(
        "POI-PAY-1",
        requests
            .get(0)
  ...
  void failedStoredValueLegReversalReportsThePartialVoid() throws Exception {
    ...
    IllegalStateException clearRefused =
        assertThrows(IllegalStateException.class, () -> session.basket().clear());
    assertTrue(
        clearRefused.getMessage().contains("voidTransaction"),
        "the error must steer the register to resuming the void: " + clearRefused.getMessage());

    SessionException endRefused = assertThrows(SessionException.class, () -> session.end().get());
    assertEquals(SessionErrorCode.INVALID_STATE, endRefused.getError().getCode());
    assertTrue(
        endRefused.getError().getMessage().contains("voidTransaction"),
        "the session must retain the in-memory void progress: "
            + endRefused.getError().getMessage());
    assertEquals(0, drainRequests().size(), "clear and end guards must not reach the wire");

    // the card leg is already reversed: a refund against it would
    // double-return the money, so refunds are refused mid-void
    SessionException refundRefused =
        assertThrows(SessionException.class, () -> session.refund(new BigDecimal("10.00")).get());
    assertEquals(SessionErrorCode.INVALID_STATE, refundRefused.getError().getCode());
    assertTrue(
        refundRefused.getError().getMessage().contains("voidTransaction"),
    ...
    assertTrue(
        session.basket().clear().isEmpty(), "the basket can be replaced after the void finishes");
  }
  ...
  void settlementCannotReplaceAPartiallyVoidedPayment() throws Exception {
    ...
    SessionException settlementRefused =
        assertThrows(SessionException.class, () -> session.settle().get());
    assertEquals(SessionErrorCode.INVALID_STATE, settlementRefused.getError().getCode());
    assertTrue(
        settlementRefused.getError().getMessage().contains("voidTransaction"),
        "the error must preserve the partial void target: "
            + settlementRefused.getError().getMessage());
    assertEquals(0, drainRequests().size(), "the replacement settlement must not reach the wire");
  }
  ...
  void voidAfterLoyaltyCheckoutReversesTheAwardByItsOwnReference() throws Exception {
    ...
    assertEquals(
        "98234",
        awardRefund.getLoyaltyRequest().getLoyaltyData()[0].getLoyaltyAccountID().getLoyaltyID(),
