...
class BasketEngineTest {
  ...
  private static BasketItem candle(int quantity) {
    return BasketItem.sale(
        "KRK-CNDL-LRG-VAN", "Large Vanilla Candle", quantity, new BigDecimal("24.99"));
  }
  ...
  void registerDiscountsCanReduceALineToZeroAndTaxTheDiscountedSubtotal() {
    BasketItem item =
        BasketItem.builder()
            ...
            .addDiscount(BasketDiscount.manual("Register markdown", new BigDecimal("5.00")))
            .addDiscount(
                BasketDiscount.offer("OFFER-1", "Free with offer", new BigDecimal("15.00")))
            .build();
  ...
  void discountsCanBeAppliedAndClearedAfterTheItemWasRung() {
    ...
    engine.setDiscountsBySku(
        "SKU-1", List.of(BasketDiscount.offer("OFFER-1", "Half off", new BigDecimal("10.00"))));
  ...
  void referencedItemsRemainDistinctInsteadOfUpsertingBySku() {
    BasketEngine engine = new BasketEngine();
    engine.addItem(
        BasketItem.sale("GIFT-CARD", "Gift card", 1, new BigDecimal("25.00"))
            .withReference("gift-card-1"));
    engine.addItem(
        BasketItem.sale("GIFT-CARD", "Gift card", 1, new BigDecimal("50.00"))
    ...
    IllegalArgumentException lookupError =
        assertThrows(IllegalArgumentException.class, () -> basket.getItemBySku("GIFT-CARD"));
    assertTrue(lookupError.getMessage().contains("use itemId or reference"));
    IllegalArgumentException mutationError =
        assertThrows(
            IllegalArgumentException.class, () -> engine.updateItemQuantityBySku("GIFT-CARD", 2));
  ...
  void addItemComputesLineAndBasketTotals() {
    ...
    assertEquals(2, basket.getItemCount());
    assertEquals(
        new BigDecimal("49.98"), basket.getItemBySku("KRK-CNDL-LRG-VAN").getOriginalTotal());
  ...
  void upsertWithDifferentUnitPriceThrows() {
    ...
    assertThrows(
        ...
        () ->
            engine.addItem(
                BasketItem.sale(
                    "KRK-CNDL-LRG-VAN", "Large Vanilla Candle", 1, new BigDecimal("19.99"))));
  }
  ...
  void taxSettersOnMissingItemsThrow() {
    BasketEngine engine = new BasketEngine();
    assertThrows(IllegalArgumentException.class, () -> engine.setTaxRate("1", BigDecimal.ONE));
    assertThrows(
        IllegalArgumentException.class, () -> engine.setTaxAmountBySku("NOPE", BigDecimal.ONE));
  }
  ...
  void snapshotsAreIndependent() {
    ...
    assertThrows(
        UnsupportedOperationException.class, () -> before.getItems().add(before.getItems().get(0)));
  }
  ...
  void returnLineNegatesTotalsAndTax() {
    ...
    assertEquals(
        new BigDecimal("24.99"), line.getUnitPrice(), "unit price stays the catalog price");
    assertEquals(new BigDecimal("-49.98"), line.getOriginalTotal());
    assertEquals(new BigDecimal("-4.44"), line.getTaxAmount(), "tax follows the line's type");
  ...
  void saleAndReturnLinesOfTheSameSkuStaySeparate() {
    ...
    Basket basket = engine.snapshot();
    assertEquals(2, basket.getItemCount(), "different item types never upsert into each other");
  ...
  void returnsAndCreditsUseSeparateSettlementSides() {
    ...
    assertEquals(
        new BigDecimal("20.00"), basket.getRefundAmount(SettlementType.REFUND_THEN_CHARGE));
  ...
  void settledChargePortionIsMergedBackIntoFullBasket() {
    ...
    BasketLineItem settledSaleLine =
  ...
  void refundAmountReflectsSettlementType() {
    ...
    refundBasket.addItem(
        BasketItem.returnItem("RETURN", "Returned item", 1, new BigDecimal("40.00")));

    BasketEngine chargeBasket = new BasketEngine();
    chargeBasket.addItem(BasketItem.sale("BUY", "New item", 1, new BigDecimal("40.00")));
    chargeBasket.addItem(
        BasketItem.returnItem("RETURN", "Returned item", 1, new BigDecimal("15.00")));

    assertEquals(
        new BigDecimal("25.00"), refundBasket.snapshot().getRefundAmount(SettlementType.NET));
    assertEquals(
        new BigDecimal("40.00"),
        refundBasket.snapshot().getRefundAmount(SettlementType.REFUND_THEN_CHARGE));
    assertEquals(BigDecimal.ZERO, chargeBasket.snapshot().getRefundAmount(SettlementType.NET));
    assertEquals(
        new BigDecimal("15.00"),
        chargeBasket.snapshot().getRefundAmount(SettlementType.REFUND_THEN_CHARGE));
  }
