...
class TerminalShopperSessionStoredValueTest {
  ...
  void activateSendsStoredValueRequestWithInitialLoad() throws Exception {
    ...
    StoredValueOperationResult result =
        session
            .storedValueActivate(
  ...
  void reverseReferencesOriginalTransaction() throws Exception {
    ...
    assertEquals(
        "POI-SV-1", data.getOriginalPOITransaction().getPoiTransactionID().getTransactionID());
  ...
  void amountValidation() {
    ...
    assertThrows(
        IllegalArgumentException.class, () -> session.storedValueLoad(card, BigDecimal.ZERO));
    assertThrows(
  ...
  void declinedOperationGoesToErrorChannel() {
    ...
    SessionException e =
        assertThrows(
            ...
            () ->
                session
                    .storedValueLoad(StoredValueCard.number("GC-1"), new BigDecimal("10.00"))
                    .get());
  ...
  void balanceUsesPaymentAccountReq() throws Exception {
    ...
    StoredValueBalance balance =
        session.storedValueBalance(StoredValueCard.number("6006491260550218157")).get();
