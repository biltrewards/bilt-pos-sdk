...
class CheckoutSessionStoredValuePurchaseTest {
    ...
    void failedLoadPreventsTheFundingCharge() throws Exception {
        ...
        assertEquals("Load", requests.get(0).getStoredValueRequest()
                .getStoredValueData()[0].getStoredValueTransactionType().toValue());
    }
    ...
    void partialActivationIsReversedWithoutCharging() throws Exception {
        ...
        assertEquals("Activate", requests.get(0).getStoredValueRequest()
                .getStoredValueData()[0].getStoredValueTransactionType().toValue());
        assertEquals("POI-LOAD-1", originalStoredValueTransaction(requests.get(1)));
        assertEquals("Reverse", requests.get(1).getStoredValueRequest()
                .getStoredValueData()[0].getStoredValueTransactionType().toValue());
    }
    ...
    void multipleStoredValueLinesKeepIndependentFulfillmentAndVoidReferences() throws Exception {
        ...
        assertEquals("Activate", purchaseRequests.get(0).getStoredValueRequest()
                .getStoredValueData()[0].getStoredValueTransactionType().toValue());
        assertEquals("Load", purchaseRequests.get(1).getStoredValueRequest()
                .getStoredValueData()[0].getStoredValueTransactionType().toValue());
    ...
    void declinedOrAbortedChargeReversesTheLoadedValue() throws Exception {
        ...
        for (String condition : List.of("Refusal", "Aborted")) {
            server.enqueue(new MockResponse().setBody(
                    storedValueOk("Load", "POI-LOAD-1", 25.00, 25.00)));
            server.enqueue(new MockResponse().setBody(paymentFailure(condition)));
            server.enqueue(new MockResponse().setBody(
                    storedValueOk("Reverse", "POI-REVERSE-1", 25.00, 0.00)));

            SessionException failure = assertThrows(SessionException.class,
                    () -> session.settle(reloadOptions()).get());

            assertEquals("Aborted".equals(condition)
                    ? SessionErrorCode.ABORTED : SessionErrorCode.DECLINED,
    ...
    void retryingTheChargeDoesNotRepeatTheLoad() throws Exception {
        ...
        SettlementResult result = session.settle(reloadOptions()).onError(failure -> {
            failures.add(failure);
            return failures.size() == 1 ? SettlementRecovery.retry() : SettlementRecovery.abort();
        }).get();
        ...
        assertEquals("POI-LOAD-1",
                failures.get(0).getCommittedMovements().get(0).getPoiTransactionId());
        List<SaleToPOIRequest> requests = drainRequests();
        assertEquals(3, requests.size());
    ...
    void failedSecondLoadReversesTheFirstWithoutCharging() throws Exception {
        ...
        session.basket().addItem(giftCard(
                "gift-card-2", "Gift card", new BigDecimal("15.00")));
        server.enqueue(new MockResponse().setBody(
                storedValueOk("Load", "POI-LOAD-1", 10.00, 10.00)));
        server.enqueue(new MockResponse().setBody(STORED_VALUE_FAILED));
        server.enqueue(new MockResponse().setBody(
                storedValueOk("Reverse", "POI-REVERSE-1", 10.00, 0.00)));
        ...
        List<SaleToPOIRequest> requests = drainRequests();
        assertEquals(3, requests.size());
        assertEquals("GC-1", requests.get(0).getStoredValueRequest()
                .getStoredValueData()[0].getStoredValueAccountID().getStoredValueID());
        assertEquals("GC-2", requests.get(1).getStoredValueRequest()
                .getStoredValueData()[0].getStoredValueAccountID().getStoredValueID());
        assertEquals("POI-LOAD-1", originalStoredValueTransaction(requests.get(2)));
    }
    ...
    void failedLoadRollbackMustFinishBeforeAnotherSettlement() throws Exception {
        ...
        server.enqueue(new MockResponse().setBody(STORED_VALUE_FAILED));
        SessionException retry = assertThrows(SessionException.class,
                () -> session.settle(reloadOptions()).get());
        ...
        server.enqueue(new MockResponse().setBody(
                storedValueOk("Reverse", "POI-REVERSE-1", 25.00, 0.00)));
        server.enqueue(new MockResponse().setBody(
                storedValueOk("Load", "POI-LOAD-2", 25.00, 25.00)));
        ...
        assertEquals("Load", recoveredRetry.get(1).getStoredValueRequest()
                .getStoredValueData()[0].getStoredValueTransactionType().toValue());
    ...
    void loadPrecedesBothSplitTenderCharges() throws Exception {
        ...
        assertEquals("Load", requests.get(0).getStoredValueRequest()
                .getStoredValueData()[0].getStoredValueTransactionType().toValue());
        assertEquals(25.00, requests.get(1).getPaymentRequest().getPaymentTransaction()
                .getAmountsReq().getRequestedAmount());
        assertEquals(15.00, requests.get(2).getPaymentRequest().getPaymentTransaction()
                .getAmountsReq().getRequestedAmount());
    }
    ...
    void externalFundingKeepsTheCompletedLoad() throws Exception {
        ...
        SettlementResult result = session.settle(reloadOptions())
    ...
    private static String paymentFailure(String condition) {
        return "{\"SaleToPOIResponse\":{\"PaymentResponse\":{"
                + "\"Response\":{\"Result\":\"Failure\",\"ErrorCondition\":\"" + condition + "\"}}}}";
    }
