...
/**
 * The session's terminal-side lifecycle bracket: the builder's {@code start()} announces the
...
class TerminalShopperSessionLifecycleTest {
  ...
  void refusedStartYieldsNoSession() {
    ...
    assertNull(sessionBuilder().start().getOrNull(), "a refused start must not hand out a session");
  }
  ...
  void throwingStartSuccessHandlerEndsTheJustStartedSession() throws Exception {
    ...
    assertEquals(
        "BiltSession,Start,v1," + delivered.get().getSessionId(),
        first.getAdminRequest().getServiceIdentification());
    assertEquals(
        "BiltSession,End,v1," + delivered.get().getSessionId(),
  ...
  void failedStartThrowsFromGet() {
    ...
    SessionException e = assertThrows(SessionException.class, () -> sessionBuilder().start().get());
  ...
  void failedForceEndStillSealsTheLocalSession() throws Exception {
    ...
    SessionException failure =
        assertThrows(
            SessionException.class, () -> session.forceEnd("terminal cleanup escalated").get());

    assertEquals(SessionErrorCode.TERMINAL_ERROR, failure.getError().getCode());
    assertEquals(
        SessionErrorCode.INVALID_STATE,
        assertThrows(SessionException.class, () -> session.identifyMember().get())
            .getError()
            .getCode());
    assertEquals(
        SessionErrorCode.INVALID_STATE,
        assertThrows(SessionException.class, () -> session.forceEnd("second attempt").get())
            .getError()
            .getCode());
    assertEquals(
        2, server.getRequestCount(), "a failed forced end cannot be retried on the sealed session");
  }
  ...
  void endedSessionRejectsEveryOperation() throws Exception {
    ...
    assertEquals(
        SessionErrorCode.INVALID_STATE,
        assertThrows(SessionException.class, () -> session.refund().get()).getError().getCode());
    assertEquals(
        SessionErrorCode.INVALID_STATE,
    ...
    assertEquals(
        SessionErrorCode.INVALID_STATE,
        assertThrows(SessionException.class, () -> session.getTransactionStatus("SVC1").get())
            .getError()
            .getCode());
    ...
    session
        .updateDisplay(DisplayPayloadHelper.standby("bye"))
    ...
    assertEquals(
        requestsBefore, server.getRequestCount(), "nothing may reach the wire after end()");
  }
