...
class CheckoutSessionBasketTest {
  ...
  void negativeTaxValuesAreRejected() throws Exception {
    ...
    assertEquals(
        0, new BigDecimal("100.00").compareTo(session.basket().snapshot().getGrandTotal()));
  }
    ...
    assertThrows(
        ...
        () ->
            session
                ...
                .mutate(
                    m ->
                        m.updateItemQuantityBySku("SKU-1", 5) // applies...
                            .removeItemBySku("NO-SUCH-SKU"))); // ...then throws

    Basket basket = session.basket().snapshot();
    assertEquals(
        2, basket.getItem("1").getQuantity(), "a failed batch must leave the basket untouched");
  }
  ...
  void designDocScanningExample() throws Exception {
    ...
    Basket basket =
        session
            ...
            .addItem(
                BasketItem.sale(
                    "KRK-CNDL-LRG-VAN", "Large Vanilla Candle", 2, new BigDecimal("24.99")));
    ...
    basket =
        session
            ...
            .addItem(
                BasketItem.sale(
                    "KRK-FRAME-5X7-BLK", "5x7 Black Frame", 1, new BigDecimal("14.99")));
  ...
  void addItemSendsItemisedReceiptDisplay() throws Exception {
    ...
    assertEquals(
        0,
        new BigDecimal("49.98").compareTo(payload.getReceipt().getTotal().getAmount().getValue()));
  }
  ...
  void mutateBatchesToSingleDisplayUpdate() throws Exception {
    ...
    Basket basket =
        session
            ...
            .mutate(
                m ->
                    m.addItem(BasketItem.sale("SKU-1", "Item One", 1, new BigDecimal("10.00")))
