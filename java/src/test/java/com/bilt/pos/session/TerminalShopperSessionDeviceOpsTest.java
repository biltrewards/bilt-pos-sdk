...
class TerminalShopperSessionDeviceOpsTest {
  ...
  void terminalOperationsKeepWorkingAfterEnd() throws Exception {
    ...
    assertNotNull(
        terminal.diagnose().get().getPoiStatus(),
  ...
  void transactionStatusOptionsControlReprintAndCategory() throws Exception {
    ...
    session
        .getTransactionStatus(
            "SVC0000001",
            TransactionStatusOptions.builder()
    ...
    assertEquals(
        "Loyalty",
        sent.getTransactionStatusRequest().getMessageReference().getMessageCategory().toValue());
    assertEquals(Boolean.TRUE, sent.getTransactionStatusRequest().getReceiptReprintFlag());
    assertEquals(
        "CustomerReceipt", sent.getTransactionStatusRequest().getDocumentQualifier()[0].toValue());
    assertEquals(
        "CashierReceipt", sent.getTransactionStatusRequest().getDocumentQualifier()[1].toValue());
  }
  ...
  void updateInputDisplayReferencesTheInFlightInput() throws Exception {
    ...
    SaleToPOIRequest inputRequest =
        mapper
            .readValue(inputRecorded.getBody().readUtf8(), NexoTerminalAPI.class)
            .getSaleToPOIRequest();
    ...
    assertEquals(
        "Input", update.getInputUpdate().getMessageReference().getMessageCategory().toValue());
    assertEquals(
        inputRequest.getMessageHeader().getServiceID(),
