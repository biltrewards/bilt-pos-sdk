...
class CheckoutSessionInputTest {
  ...
  void digitStringSendsPromptAndParsesResult() throws Exception {
    server.enqueue(
        new MockResponse()
            .setBody(inputResponse("\"InputCommand\":\"DigitString\",\"DigitInput\":\"10001\"")));

    String zip =
        session
            .requestDigitString(
                "Enter your zip code", InputOptions.builder().minLength(5).maxLength(5).build())
            .get();
    ...
    InputPayload payload =
        DisplayPayloadHelper.inputFromBase64(
  ...
  void decimalStringParsesAmount() {
    server.enqueue(
        new MockResponse()
            .setBody(inputResponse("\"InputCommand\":\"DecimalString\",\"DigitInput\":\"4.50\"")));
  ...
  void textStringParsesText() {
    server.enqueue(
        new MockResponse()
            .setBody(inputResponse("\"InputCommand\":\"TextString\",\"TextInput\":\"a@b.com\"")));
  ...
  void confirmationParsesFlagAndSendsCustomButtons() throws Exception {
    server.enqueue(
        new MockResponse()
            .setBody(inputResponse("\"InputCommand\":\"GetConfirmation\",\"ConfirmedFlag\":true")));

    Boolean confirmed =
        session
            .requestConfirmation(
                "Print receipt?", ConfirmationOptions.withButtons("Print", "No thanks"))
            .get();
  ...
  void menuEntryMapsOneBasedWireSelectionToZeroBasedIndex() throws Exception {
    server.enqueue(
        new MockResponse()
            .setBody(inputResponse("\"InputCommand\":\"GetMenuEntry\",\"MenuEntryNumber\":[2]")));

    MenuSelection selection =
        session.requestMenuEntry("Select tip", List.of("15%", "18%", "20%")).get();
    ...
    assertEquals(
        "18%",
        sent.getInputRequest().getDisplayOutput().getMenuEntry()[1].getOutputText()[0].getText());
    assertEquals(
        1L,
        sent.getInputRequest().getInputData().getMaxLength(),
  ...
  void menuEntryMultiSelectReturnsAllEntries() throws Exception {
    server.enqueue(
        new MockResponse()
            .setBody(inputResponse("\"InputCommand\":\"GetMenuEntry\",\"MenuEntryNumber\":[1,3]")));
    ...
    MenuSelection selection =
        session
            .requestMenuEntry(
                "Pick",
                List.of("A", "B", "C"),
                com.bilt.pos.session.input.MenuOptions.builder().multiSelect(true).build())
            .get();
  ...
  void cancelledInputMapsToCancelledError() {
    ...
    SessionException e =
        assertThrows(SessionException.class, () -> session.requestDigitString("Zip?").get());
  ...
  void declinedSignatureMapsToCancelled() {
    ...
    SessionException e =
        assertThrows(SessionException.class, () -> session.requestSignature("Sign").get());
    assertEquals(SessionErrorCode.CANCELLED, e.getError().getCode());
  }
  ...
  void pinEntrySendsPinRequestOnTerminal() throws Exception {
    ...
    PinResult result =
        session
            .requestPinEntry(PinOptions.builder().timeout(java.time.Duration.ofSeconds(30)).build())
            .get();
  ...
  void inputGoesToExternalDisplayButPinStaysOnTerminal() throws Exception {
    try (MockWebServer displayServer = new MockWebServer()) {
      ...
      assertEquals(
          3, server.getRequestCount(), "both session starts and the PIN must stay on the terminal");
    }
  }
