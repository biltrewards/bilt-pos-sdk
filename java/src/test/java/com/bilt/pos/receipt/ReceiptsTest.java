...
class ReceiptsTest {
    ...
    NexoTerminalAPI response =
        NexoTerminalAPI.builder()
            .saleToPOIResponse(
                SaleToPOIResponse.builder()
                    .reversalResponse(
                        ReversalResponse.builder()
                            .paymentReceipt(
                                new PaymentReceipt[] {
                                  buildPaymentReceipt(
                                      DocumentQualifierEnum.CUSTOMER_RECEIPT,
                                      sampleReceiptBase64("CUSTOMER"))
                                })
                            .build())
                    .build())
            .build();
  ...
  void fromPaymentReceiptsShouldSkipEntriesWithNullQualifier() throws JAXBException {
    PaymentReceipt pr =
        PaymentReceipt.builder()
            .outputContent(
                OutputContent.builder().outputXHTML(sampleReceiptBase64("CUSTOMER")).build())
            .build();
  ...
  void fromPaymentReceiptsShouldSkipEntriesWithNullOutputContent() throws JAXBException {
    PaymentReceipt pr =
        PaymentReceipt.builder().documentQualifier(DocumentQualifierEnum.CUSTOMER_RECEIPT).build();
  ...
  void receiptDataShouldBeAccessibleFromExtractedReceipt() throws JAXBException {
    ...
    NexoTerminalAPI response =
        buildPaymentResponse(
            buildPaymentReceipt(
                DocumentQualifierEnum.CUSTOMER_RECEIPT, ReceiptHelper.toBase64(receipt)));
  ...
  private NexoTerminalAPI buildPaymentResponse(PaymentReceipt... receipts) {
    return NexoTerminalAPI.builder()
        .saleToPOIResponse(
            SaleToPOIResponse.builder()
                .paymentResponse(PaymentResponse.builder().paymentReceipt(receipts).build())
                .build())
        .build();
  }
