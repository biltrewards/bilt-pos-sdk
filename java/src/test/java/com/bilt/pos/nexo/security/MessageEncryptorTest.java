...
class MessageEncryptorTest {
  ...
  void encryptedMessageHasExpectedCmsStructure() throws Exception {
    ...
    assertEquals(
        MessageEncryptor.ALG_HMAC_SHA256,
  ...
  void decryptFailsOnTamperedCiphertext() throws Exception {
    ...
    byte[] ciphertext =
        Base64.getDecoder()
            .decode(message.getEnvelopedData().getEncryptedContent().getEncryptedData());
    ciphertext[0] ^= 0xFF;
    message
        .getEnvelopedData()
        .getEncryptedContent()
    ...
    assertThrows(EncryptionException.class, () -> encryptor.decrypt(message, headerBytes));
  }
  ...
  void decryptFailsOnTamperedHeader() throws Exception {
    ...
    assertThrows(EncryptionException.class, () -> encryptor.decrypt(message, tamperedHeaderBytes));
  }
  ...
  void differentKeyCannotDecrypt() throws Exception {
    ...
    assertThrows(EncryptionException.class, () -> otherEncryptor.decrypt(message, headerBytes));
  }
  ...
  void securityKeyRequiresPassphrase() {
    assertThrows(
        IllegalStateException.class, () -> SecurityKey.builder().keyIdentifier("test").build());
  }

  @Test
  void securityKeyRequiresKeyIdentifier() {
    assertThrows(
        IllegalStateException.class, () -> SecurityKey.builder().passphrase("test").build());
  }
}
