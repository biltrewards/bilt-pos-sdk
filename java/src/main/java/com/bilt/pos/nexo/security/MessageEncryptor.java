 ...
 *
 * <p>Key material is derived from the {@link SecurityKey} passphrase via PBKDF2. The derived cipher
 * key acts as the KEK (Key Encryption Key) for wrapping per-message session keys. The derived HMAC
 * key is used directly for message authentication.
 *
 * <p>Each message uses a fresh random AES-256 session key, wrapped by the KEK using AES Key Wrap
 * (RFC 3394). The HMAC covers both the serialized message header and the plaintext body.
 *
...
public final class MessageEncryptor {
  ...
  public SaleToPOISecuredMessage encrypt(String jsonPayload, MessageHeader messageHeader)
    ...
    try {
      ...
      KEKIdentifier kekId =
          KEKIdentifier.builder()
              .keyIdentifier(securityKey.getKeyIdentifier())
              .keyVersion(String.valueOf(securityKey.getKeyVersion()))
              .build();

      AlgorithmIdentifier keyWrapAlg =
          AlgorithmIdentifier.builder().algorithm(ALG_AES256_KEY_WRAP).build();
      ...
      EnvelopedData envelopedData =
          EnvelopedData.builder()
              ...
              .kek(
                  buildKek(
                      kekId, keyWrapAlg, Base64.getEncoder().encodeToString(wrappedSessionKey)))
              .encryptedContent(
                  EncryptedContent.builder()
                      .contentType(EncapsulatedContentContentType.ID_DATA)
                      .contentEncryptionAlgorithm(
                          AlgorithmIdentifier.builder()
      ...
      ContentInformationType securityTrailer =
          ContentInformationType.builder()
              .contentType(SecurityTrailerContentType.ID_CT_AUTH_DATA)
              .authenticatedData(
                  AuthenticatedData.builder()
                      .version(AuthenticatedDataVersion.V0)
                      .kek(buildKek(kekId, keyWrapAlg, ""))
                      .macAlgorithm(
                          AlgorithmIdentifier.builder().algorithm(ALG_HMAC_SHA256).build())
                      .encapsulatedContent(
                          EncapsulatedContent.builder()
                              .contentType(EncapsulatedContentContentType.ID_DATA)
                              .build())
   ...
   *
   * <p>The {@code rawHeaderBytes} must be the exact bytes of the {@code MessageHeader} JSON as
   * received on the wire (before deserialization), since the HMAC is computed over the raw header
   * bytes concatenated with the plaintext body. Re-serializing the deserialized Java object could
   * produce different byte output if the sender's JSON library uses a different field ordering,
   * which would cause HMAC verification to fail.
   *
  ...
  public String decrypt(SaleToPOISecuredMessage message, byte[] rawHeaderBytes)
    ...
    try {
      ...
      byte[] wrappedSessionKey =
          Base64.getDecoder().decode(envelopedData.getKek().getEncryptedKey());
      ...
      byte[] ciphertext =
          Base64.getDecoder().decode(envelopedData.getEncryptedContent().getEncryptedData());
      ...
      byte[] receivedHmac =
          Base64.getDecoder().decode(message.getSecurityTrailer().getAuthenticatedData().getMAC());
  ...
  private static Kek buildKek(
      KEKIdentifier kekId, AlgorithmIdentifier keyWrapAlg, String encryptedKey) {
    return Kek.builder()
