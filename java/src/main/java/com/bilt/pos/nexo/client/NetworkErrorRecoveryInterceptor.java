...
/**
 * OkHttp application interceptor that recovers an in-flight request across a transient network
 * failure without re-executing the operation on the terminal.
 *
 * <p>Each logical request is tagged with a stable {@value #REQUEST_ID_HEADER} UUID. On any {@link
 * IOException} the interceptor re-sends the <em>identical</em> request (same id) after a backoff,
 * until a response arrives or the operation timeout elapses. The terminal keys execution on the id
 * and treats a request whose id it already knows as a recovery rather than a new operation, so
 * re-sending is always safe (at-most-once execution).
 *
 * <p>The overall recovery budget is the call's read timeout ({@link Chain#readTimeoutMillis()}),
 * treated as the timeout for the whole operation; overriding the timeout (on the client or per
 * request) changes the budget with it, and {@code 0} ("no timeout") means recovery retries until it
 * succeeds. Each individual attempt is additionally capped at a shorter per-attempt timeout —
 * derived from the read timeout as {@code max(readTimeout / 6, min(10s, readTimeout))} (≈20s at the
 * 120s default) — via {@link Chain#withReadTimeout}, so a single stuck attempt — e.g. a TLS
 * handshake to a silently black-holed peer, which OkHttp otherwise bounds by the full read timeout
 * — cannot consume the whole budget.
 *
 * <p>This class is intentionally package-private: it is wired in by the client whenever recovery is
 * active (on by default; see {@link
 * BiltNexoTerminalClient.Builder#disableRecoveryOnNetworkError()}) and is not part of the public
 * API. See {@code docs/network-error-recovery.md} for the full client/server contract.
 */
final class NetworkErrorRecoveryInterceptor implements Interceptor {
  ...
  /**
   * Derive the per-attempt timeout from the overall (read) timeout: {@code max(readTimeout / 6,
   * min(10s, readTimeout))}. At least ~10s (or the whole timeout if it is shorter), otherwise a
   * sixth of the total — roughly six attempts for a large timeout. A non-positive ("no timeout")
   * read timeout yields the 10s floor so black holes stay bounded.
   */
  ...
  public Response intercept(Chain chain) throws IOException {
    ...
    Request request = chain.request().newBuilder().header(REQUEST_ID_HEADER, requestId).build();
    ...
    while (true) {
      ...
      if (budgetNanos >= 0) {
        long remainingMillis =
            TimeUnit.NANOSECONDS.toMillis(budgetNanos - (System.nanoTime() - startNanos));
      ...
      try {
        ...
        return chain.withReadTimeout(attemptTimeoutMillis, TimeUnit.MILLISECONDS).proceed(request);
      } catch (IOException e) {
        ...
        if (budgetNanos >= 0) {
          ...
          sleepMillis = Math.min(sleepMillis, TimeUnit.NANOSECONDS.toMillis(remainingNanos));
        }
  ...
  /**
   * True for failures that a retry cannot fix: TLS identity/trust failures and protocol errors.
   * Mirrors OkHttp's own non-recoverable classification so recovery does not pointlessly re-attempt
