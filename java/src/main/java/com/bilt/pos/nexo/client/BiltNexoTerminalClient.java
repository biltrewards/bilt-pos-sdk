 ...
 * (v3.0).
 *
 * <p>The terminal exposes a single HTTPS endpoint at {@code https://<device_ip>:8443/nexo}. Each
 * call sends a {@link NexoTerminalAPI} containing a {@link SaleToPOIRequest} and receives a {@link
 * NexoTerminalAPI} containing a {@link SaleToPOIResponse}.
 *
 ...
 *
 * <p>Rather than disabling verification with {@link Builder#trustAllCertificates()}, trust the
 * merchant (or small-merchant-pool) CA certificate so any device leaf certificate issued under it
 * is accepted, and verify the device identity against the certificate's synthetic hostname instead
 * of the connection IP:
 *
 * <pre>{@code
 * BiltNexoTerminalClient client = BiltNexoTerminalClient.builder()
 *     .endpoint("https://192.168.1.100:8443/nexo")
...
public final class BiltNexoTerminalClient implements TerminalClient {
  ...
  private BiltNexoTerminalClient(Builder builder) {
    ...
    OkHttpClient baseHttpClient =
        builder.httpClient != null ? builder.httpClient : createDefaultHttpClient(builder);
    this.httpClient =
        builder.recoverOnNetworkError
            ? baseHttpClient
                .newBuilder()
                .addInterceptor(new NetworkErrorRecoveryInterceptor())
                .build()
            : baseHttpClient;
    this.encryptor = builder.securityKey != null ? new MessageEncryptor(builder.securityKey) : null;
   ...
   *
   * <p>If a {@link SecurityKey} was provided to the builder, the request is automatically encrypted
   * and the response is decrypted.
   *
   * @param request the request envelope; must have {@code saleToPOIRequest} set
   * @return the terminal's response envelope containing {@code saleToPOIResponse}, or {@code null}
  ...
  public NexoTerminalAPI request(NexoTerminalAPI request, Duration timeout)
    ...
    try {
      String plainRequest =
          encryptor == null || messageListener != null
      ...
      try {
        ...
        notifyMessage(NexoMessageListener.Direction.RESPONSE, responseJson);
        throw new BiltNexoClientException("Failed to parse terminal response: " + responseJson, e);
      }
      ...
      throw new BiltNexoClientException("Failed to communicate with terminal at " + endpoint, e);
    }
  }
  ...
  /**
   * Convenience factory for an unencrypted client pointing at {@code https://<host>:8443/nexo}.
   *
   * @param host the terminal's IP address or hostname
   * @return a new client instance
   */
  public static BiltNexoTerminalClient create(String host) {
    return builder().endpoint("https://" + host + ":" + DEFAULT_PORT + DEFAULT_PATH).build();
  }
  ...
  private String encryptRequest(SaleToPOIRequest request) throws IOException, EncryptionException {
    ...
    SaleToPOISecuredMessage secured = encryptor.encrypt(plainJson, request.getMessageHeader());
  ...
  private ParsedResponse parseResponse(String responseJson)
    ...
    if (secured != null && secured.getEnvelopedData() != null) {
      ...
      JsonNode responseNode = objectMapper.readTree(responseJson).get("SaleToPOIResponse");
      ...
      return new ParsedResponse(
          NexoTerminalAPI.builder().saleToPOIResponse(saleToPOIResponse).build(), plainJson);
    } else {
      return new ParsedResponse(
          objectMapper.readValue(responseJson, NexoTerminalAPI.class), responseJson);
    }
  }
  ...
  public static final class Builder {
    ...
    /** Set the full terminal endpoint URL (e.g. {@code https://192.168.1.100:8443/nexo}). */
    ...
    /**
     * Set the read timeout (time waiting for the terminal to respond). Default is 120 seconds,
     * which accommodates cardholder interaction during payment transactions.
     */
    ...
    /**
     * Set the interval at which the client sends keep-alive pings to the terminal while a
     * connection is held open. Use this to detect a dead terminal during long waits (e.g.
     * cardholder interaction) rather than blocking until the read timeout elapses.
     *
     * <p>Pings are HTTP/2 PING frames, so this only has an effect when the connection is negotiated
     * as HTTP/2 via ALPN; on an HTTP/1.1 connection the interval is ignored. {@link Duration#ZERO}
     * (the default) disables pinging.
     *
     * <p>If the terminal does not acknowledge a ping within the interval, the connection is
     * considered failed and the in-flight request fails with an {@code IOException} (surfaced as a
     * {@link BiltNexoClientException}).
     *
     * @param pingInterval the ping interval, e.g. {@code Duration.ofSeconds(1)}; {@code
     *     Duration.ZERO} disables pinging
     */
     ...
     *
     * <p>Recovery is <strong>on by default</strong>. While on, each request is tagged with a stable
     * {@code X-Bilt-Request-Id} correlation UUID; if the connection is torn down by a transient
     * network error while the request is in flight, the client re-sends the identical request (same
     * id) with backoff until it obtains the response or the operation (read) timeout elapses. The
     * terminal keys execution on the id and treats an already-seen id as a recovery rather than a
     * new operation, so the underlying operation runs at most once no matter how many times the
     * client re-attempts. (Only transient failures are retried; deterministic ones such as a TLS
     * trust failure are surfaced immediately.)
     *
     * <p><strong>Recovery requires terminal-side support for the correlation protocol</strong> (see
     * {@code docs/network-error-recovery.md}). Against a terminal that does not implement it, a
     * re-send is treated as a new operation, which for a long-running request risks executing it
     * more than once — call this to turn recovery off in that case.
     *
     * <p>Turning recovery off also removes the keep-alive {@link #pingInterval(Duration)} that
     * recovery otherwise enables (unless a ping interval was set explicitly).
     */
    ...
    /**
     * Enable Nexo message encryption. When set, all requests are AES-encrypted and all responses
    ...
    /**
     * Observe plaintext Nexo request and response JSON for diagnostics. Requests are delivered
     * before encryption and valid Nexo responses after decryption. Malformed or unsuccessful HTTP
     * response bodies are delivered as received. Successful responses retain the terminal's exact
     * plaintext, including unknown fields and formatting. Listener failures do not fail the
     * terminal operation. Payloads may contain cardholder data, including full PANs; do not persist
     * or log them in production builds. The listener runs synchronously on the requesting thread,
     * so callbacks should hand off UI or long-running work and return quickly.
     */
    ...
    /**
     * Disable TLS certificate verification. <strong>For development and testing only</strong> — the
     * terminal uses a self-signed certificate.
     */
    ...
    /**
     * Add a trust anchor: trust the terminal's TLS certificate by supplying the certificate it
     * chains to — in production this is the merchant or small-merchant-pool CA certificate, so
     * every device leaf certificate issued under that CA is accepted.
     *
     * <p>Unlike {@link #trustAllCertificates()}, the TLS handshake chain is fully verified: only
     * certificates that chain to one of the supplied anchors are accepted. Because Bilt terminals
     * present a leaf certificate whose Subject Alternative Name is a synthetic, non-resolving
     * hostname ({@code {Model}-{Serial}.live.pos.bilt.com}) while the client connects by IP
     * address, pair this with {@link #expectedHostnamePattern(String)} so the certificate identity
     * is verified against the SAN rather than the connection address.
     *
     * <p>This method may be called multiple times to add several anchors.
     *
    ...
    public Builder trustCertificate(Path certificatePath) {
      try (InputStream in = Files.newInputStream(certificatePath)) {
        ...
        throw new IllegalArgumentException("Failed to read certificate from " + certificatePath, e);
      }
    }

    /**
     * Trust the terminal's TLS certificate by pinning the supplied public certificate file. See
    ...
    /**
     * Trust the terminal's TLS certificate by pinning a certificate loaded from the given classpath
     * resource (e.g. one bundled in {@code src/main/resources}). See {@link
    ...
    public Builder trustCertificateResource(String resourceName) {
      InputStream in =
          BiltNexoTerminalClient.class.getClassLoader().getResourceAsStream(resourceName);
      if (in == null) {
    ...
    /**
     * Trust the terminal's TLS certificate by pinning one or more X.509 certificates read from the
     * supplied stream. See {@link #trustCertificate(Path)}.
     *
     * <p>The stream is consumed but not closed; the caller owns its lifecycle.
     *
    ...
    public Builder trustCertificate(InputStream certificateStream) {
      try {
        ...
        Collection<? extends Certificate> certs = factory.generateCertificates(certificateStream);
        if (certs.isEmpty()) {
          throw new IllegalArgumentException("No X.509 certificate found in the supplied stream");
        }
    ...
    /**
     * Trust the terminal's TLS certificate by pinning an already-loaded {@link X509Certificate}.
     * See {@link #trustCertificate(Path)}.
     */
    ...
    /**
     * Verify the terminal's identity against the standard hostname pattern for the given Bilt
     * environment, instead of matching the connection's IP address. Equivalent to {@code
     * expectedHostnamePattern( environment.hostnamePattern())}.
     *
     * <p>The caller must still trust the CA hierarchy for the <em>same</em> environment via {@link
     * #trustCertificate(Path)}; production and staging use fully separate CA roots, so the trust
     * anchor and the environment must agree.
     *
    ...
    /**
     * Verify the terminal's identity by matching the certificate's Subject Alternative Name (DNS
     * entries, falling back to the Common Name) against the given pattern, instead of matching the
     * connection's IP address.
     *
     * <p>Bilt terminals are reached by IP address but present a leaf certificate carrying a
     * synthetic, non-resolving hostname of the form {@code {Model}-{Serial}.live.pos.bilt.com}.
     * Standard hostname verification would reject this because the connection host (the IP) never
     * matches that name. Setting a pattern such as {@code "*.live.pos.bilt.com"} replaces the
     * default verifier with one that accepts any terminal whose certificate name matches the
     * pattern.
     *
     * <p>The pattern supports a single leading wildcard label, e.g. {@code "*.live.pos.bilt.com"}
     * matches {@code "V240m-ABC123.live.pos.bilt.com"} (one label before the suffix) but not {@code
     * "a.b.live.pos.bilt.com"}. A pattern without a wildcard is matched exactly. Matching is
     * case-insensitive.
     *
     * <p>This controls identity verification only; the certificate chain is still validated against
     * the anchors added with {@link #trustCertificate(Path)}. It has no effect when {@link
     * #trustAllCertificates()} is set.
     *
    ...
    /**
     * Provide a custom {@link OkHttpClient}. When set, the timeout and TLS settings on this builder
     * are ignored.
     */
    ...
    /**
     * Provide a custom {@link ObjectMapper}. When not set, a default mapper is created that omits
     * null fields and ignores unknown properties.
     */
  ...
  /**
   * Build an {@link EventListener} that logs the negotiated application protocol (HTTP/2 vs
   * HTTP/1.1) once a connection is acquired. The protocol is chosen by the terminal via ALPN during
   * the TLS handshake; this makes the outcome visible at {@code FINE} (e.g. under the CLI's {@code
   * --verbose} flag) without changing the request/response API.
   */
    ...
    return new EventListener() {
      ...
      public void connectionAcquired(Call call, Connection connection) {
        if (LOG.isLoggable(Level.FINE)) {
          LOG.fine(
              "Terminal connection negotiated "
                  + connection.protocol()
                  + " with "
  ...
  /**
   * Build a trust manager that accepts only certificates chaining to one of the supplied trust
   * anchors. The anchors are loaded into an in-memory {@link KeyStore} as trusted entries.
   */
  ...
  /**
   * Build a hostname verifier that ignores the connection address and instead matches the peer leaf
   * certificate's DNS SANs (falling back to the CN) against {@code pattern}. The certificate chain
   * is validated separately by the trust manager; this only checks identity.
   */
  ...
  /**
   * Match a certificate name against a pattern that supports a single leading wildcard label (e.g.
   * {@code "*.live.pos.bilt.com"} matches one label before the suffix). Patterns without a wildcard
   * are matched exactly. Matching is case-insensitive.
   */
