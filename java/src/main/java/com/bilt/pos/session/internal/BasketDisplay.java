...
/**
 * The customer-display side of a session: renders basket snapshots through the configured {@link
 * DisplayRenderer} and sends the payload as a Nexo {@code DisplayRequest}. Sale, return, and credit
 * lines go through the same pipeline; return and credit lines render negative.
 *
 * <p>Failures — renderer, serialization, or wire — surface as {@link SessionException}; whether a
 * display update is best-effort is the caller's policy, not this class's.
 */
public final class BasketDisplay {
  ...
  /**
   * Renders the basket and sends it to the customer display. A renderer returning {@code null}
   * means "nothing to show" and skips the send.
   */
  public void show(Basket basket) {
    ...
    try {
      ...
      throw new SessionException(
          new SessionError(
              SessionErrorCode.UNKNOWN, "the display renderer failed: " + e.getMessage(), null, e));
    }
    if (payload != null) {
      send(payload);
    }
  ...
  public void send(DisplayPayload payload) {
    ...
    try {
      ...
      throw new SessionException(
          new SessionError(
              SessionErrorCode.UNKNOWN, "failed to serialize the display payload", null, e));
    }
    exchange.send(MessageCategoryType.DISPLAY, exchange.factory().displayRequest(base64));
  }
}
