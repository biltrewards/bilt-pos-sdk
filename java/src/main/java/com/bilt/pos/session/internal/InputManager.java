...
/**
 * Builds Nexo {@code InputRequest}/{@code PINRequest} messages and parses their responses into the
 * typed values of the session input API.
 */
public final class InputManager {
  ...
  public String digitString(String prompt, InputOptions options) {
    Input input =
        requestInput(
            InputCommandEnum.DIGIT_STRING,
            displayPayload(prompt, options),
            inputData(options),
            options.getTimeout());
  ...
  public BigDecimal decimalString(String prompt, InputOptions options) {
    Input input =
        requestInput(
            InputCommandEnum.DECIMAL_STRING,
            displayPayload(prompt, options),
            inputData(options),
            options.getTimeout());
    ...
    try {
      ...
      throw new SessionException(
          new SessionError(
              ...
              "terminal returned a non-decimal value: " + value,
              null,
              e));
    }
  }
  ...
  public boolean confirmation(String prompt, ConfirmationOptions options) {
    ...
    Input input =
        requestInput(
            InputCommandEnum.GET_CONFIRMATION, payload, InputData.builder(), options.getTimeout());
  ...
  public MenuSelection menuEntry(String prompt, List<String> entries, MenuOptions options) {
    InputPayload payload =
        options.getAdditionalText() != null
    ...
    for (int i = 0; i < entries.size(); i++) {
      menuEntries[i] =
          MenuEntry.builder()
              ...
              .outputText(new OutputText[] {OutputText.builder().text(entries.get(i)).build()})
              .build();
    }
    ...
    SaleToPOIRequest request =
        inputRequest(InputCommandEnum.GET_MENU_ENTRY, payload, menuData, menuEntries);
    Duration httpTimeout =
        options.getTimeout() == null ? null : options.getTimeout().plusSeconds(10);
    Input input =
        parseInput(exchange.sendExpectingBody(MessageCategoryType.INPUT, request, httpTimeout));
    ...
    for (long number : selected) {
      ...
      if (index < 0 || index >= entries.size()) {
        throw new SessionException(
            new SessionError(
                SessionErrorCode.TERMINAL_ERROR,
                "terminal selected menu entry " + number + " outside the offered range"));
      }
      indices.add(index);
  ...
  public Signature signature(String prompt) {
    SaleToPOIRequest request =
        inputRequest(
            InputCommandEnum.GET_CONFIRMATION,
            DisplayPayloadHelper.signature(prompt),
            InputData.builder(),
            null);
    ...
    if (!Boolean.TRUE.equals(input.getConfirmedFlag())) {
      throw new SessionException(
          new SessionError(SessionErrorCode.CANCELLED, "the customer declined to sign"));
    }
    String image =
        response.getInputResponse().getInputResult().getResponse().getAdditionalResponse();
    if (image == null) {
  ...
  public boolean amountConfirmation(BigDecimal amount, String prompt, String currency) {
    ...
    Input input =
        requestInput(InputCommandEnum.GET_CONFIRMATION, payload, InputData.builder(), null);
    return Boolean.TRUE.equals(input.getConfirmedFlag());
  }
  ...
  public PinResult pin(PinMode mode, PinOptions options) {
    PINRequest.Builder pinRequest = PINRequest.builder().pinRequestType(toWire(mode));
    if (options.getTimeout() != null) {
    ...
    SaleToPOIRequest request =
        SaleToPOIRequest.builder()
            .messageHeader(
                exchange.factory().header(MessageClassType.DEVICE, MessageCategoryType.PIN))
    ...
    boolean verified =
        mode != PinMode.PIN_ENTER && body.getResponse().getResult() != ResultType.FAILURE;
  ...
  private Input requestInput(
    ...
    return parseInput(exchange.sendExpectingBody(MessageCategoryType.INPUT, request, httpTimeout));
  }

  private SaleToPOIRequest inputRequest(
      InputCommandEnum command,
      InputPayload payload,
      InputData.Builder inputData,
    ...
    try {
      ...
      throw new SessionException(
          new SessionError(SessionErrorCode.UNKNOWN, "failed to serialize input payload", null, e));
    }
    DisplayOutput.Builder displayOutput =
    ...
    return SaleToPOIRequest.builder()
        .messageHeader(
            exchange.factory().header(MessageClassType.DEVICE, MessageCategoryType.INPUT))
        .inputRequest(
