 ...
 *
 * <p>{@link #awaitCall} runs the handler on the callback executor and waits for it, so a handler
 * behaves exactly as if invoked inline — same ordering, same exception propagation, and a returned
 * value can steer the flow that asked — while physically running on the integrator's thread. It
 * must only be called from a session's operation thread: awaiting from the very thread the callback
 * executor dispatches to would deadlock.
 *
 * <p>{@link #fireAndForget} is for the one dispatch that happens on the caller's own thread — an
 * operation rejected at submission — where awaiting could deadlock a caller that is itself the
 * callback thread.
 *
 * <p>Everywhere, a {@code null} executor means direct delivery (run in place), and a callback
 * executor that rejects the dispatch falls back to running in place rather than costing the {@code
  ...
  static <R> R awaitCall(Executor callback, String operationName, Supplier<R> handler) {
    ...
    try {
      ...
      LOGGER.log(
          Level.WARNING,
          operationName
              + "'s callback executor rejected the "
              + "dispatch; running the handler on the session thread",
          e);
      return handler.get();
    }
    try {
  ...
  static void fireAndForget(Executor callback, String operationName, Runnable dispatch) {
    ...
    try {
      ...
      LOGGER.log(
          ...
          operationName
              + "'s callback executor rejected the "
              + "dispatch; delivering on the calling thread",
          e);
      dispatch.run();
    }
  }
