 ...
 *
 * <p>The terminal confirmation screen has two buttons; custom labels replace the default
 * confirm/cancel texts. For three or more choices use {@code requestMenuEntry} instead.
 */
public final class ConfirmationOptions {
  ...
  /**
   * Custom button labels: the first is the confirm button, the optional second the cancel button.
   *
   * @throws IllegalArgumentException for more than two labels — use {@code requestMenuEntry} for
  ...
  public static ConfirmationOptions withButtons(String... buttons) {
    if (buttons.length == 0 || buttons.length > 2) {
      throw new IllegalArgumentException(
          "expected 1 or 2 button labels; for " + buttons.length + " choices use requestMenuEntry");
    }
    return new ConfirmationOptions(buttons[0], buttons.length > 1 ? buttons[1] : null, null);
  }
