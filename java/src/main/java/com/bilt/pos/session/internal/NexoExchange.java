...
/**
 * Sends session requests through the {@link DisplayRouter} and translates the two failure layers —
 * transport exceptions and in-body Nexo result codes — into {@link SessionError}s.
 *
 * <p>Also tracks the in-flight operation so {@code abort()} can reference it from another thread.
 */
public final class NexoExchange {
  ...
  /**
   * Sends a Bilt session lifecycle signal as a Nexo {@code AdminRequest} (see {@link
   * SessionSignalCodec}), requiring success.
   */
  public void sendSessionSignal(String serviceIdentification) {
    SaleToPOIRequest request =
        SaleToPOIRequest.builder()
            .messageHeader(factory.header(MessageClassType.SERVICE, MessageCategoryType.ADMIN))
            .adminRequest(
                AdminRequest.builder().serviceIdentification(serviceIdentification).build())
            .build();
  ...
  /**
   * Best-effort Nexo {@code AbortRequest} for the operation currently awaiting a response, sent to
   * the device processing it. No-op with nothing in flight. The session lifecycle signals (the
   * ADMIN category carries only start/end) are never the abort's target: cancelling an end exchange
   * would buy nothing and could strand the terminal's session-scoped data.
   */
  ...
  /**
   * Sends {@code request} to the client owning {@code category} and returns the response body,
   * which may be {@code null} when the terminal replies with an empty body (e.g. for aborts).
   *
   * @throws SessionException with {@link SessionErrorCode#NETWORK} or {@link
   *     SessionErrorCode#TIMEOUT} on transport failure
   */
  ...
  public SaleToPOIResponse send(
    ...
    try {
      ...
      NexoTerminalAPI response =
          timeout == null ? client.request(envelope) : client.request(envelope, timeout);
  ...
  /**
   * Like {@link #send} but treats an empty response body as a {@link
   * SessionErrorCode#TERMINAL_ERROR}.
   */
  public SaleToPOIResponse sendExpectingBody(
  ...
  /**
   * Resolves an earlier request by ServiceID. Returns the repeated original response, or {@code
   * null} when the terminal authoritatively reports that it was not found. An in-progress response
   * is polled for a bounded period; any other status exchange failure remains a SessionException.
   */
  public SaleToPOIResponse recoverByTransactionStatus(
      String serviceId, MessageCategoryType originalCategory) {
    for (int attempt = 1; attempt <= TRANSACTION_STATUS_MAX_ATTEMPTS; attempt++) {
        ...
        if (attempt == TRANSACTION_STATUS_MAX_ATTEMPTS) {
          throw new SessionException(
              new SessionError(
                  ...
                  "terminal is still processing "
                      + originalCategory.toValue()
                      + " request "
                      + serviceId
                      + " after "
                      + attempt
                      + " TransactionStatus attempts",
                  ErrorConditionType.IN_PROGRESS.toValue(),
                  null));
        }
      ...
      if (repeated == null) {
        throw Wire.missing("TransactionStatusResponse.RepeatedMessageResponse");
      }
      boolean expectedBodyPresent =
          (originalCategory == MessageCategoryType.PAYMENT && repeated.getPaymentResponse() != null)
      ...
      if (!expectedBodyPresent) {
        throw Wire.missing(
            "TransactionStatusResponse repeated " + originalCategory.toValue() + " response");
      }
  ...
  private SaleToPOIRequest transactionStatusRequest(
      String serviceId, MessageCategoryType originalCategory) {
    return SaleToPOIRequest.builder()
        .messageHeader(
            factory.header(MessageClassType.SERVICE, MessageCategoryType.TRANSACTION_STATUS))
  ...
  private static void waitBeforeStatusRetry() {
    long delayMillis =
        ThreadLocalRandom.current()
            .nextLong(
                TRANSACTION_STATUS_POLL_INTERVAL_MILLIS - TRANSACTION_STATUS_POLL_JITTER_MILLIS,
    ...
    try {
      ...
      throw new SessionException(
          new SessionError(
              SessionErrorCode.TERMINAL_ERROR,
              "TransactionStatus recovery was interrupted",
              null,
              e));
    }
  }
  ...
  /**
   * Throws a {@link SessionException} when the in-body result reports failure. A missing {@code
   * Response} object is treated as a terminal error; {@code Partial} results pass.
   */
  ...
  private static SessionErrorCode toErrorCode(
      MessageCategoryType category, ErrorConditionType condition) {
    if (condition == null) {
  ...
  private static SessionError toTransportError(
      MessageCategoryType category, BiltNexoClientException e) {
    for (Throwable t = e; t != null; t = t.getCause()) {
      if (t instanceof SocketTimeoutException || t instanceof InterruptedIOException) {
        return new SessionError(
            SessionErrorCode.TIMEOUT,
            category.toValue() + " request timed out: " + e.getMessage(),
            null,
            e);
      }
    }
    return new SessionError(
        ...
        category.toValue() + " request failed: " + e.getMessage(),
        null,
        e);
  }
}
