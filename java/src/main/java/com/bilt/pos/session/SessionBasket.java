 ...
 * session has automatic display enabled it also enqueues an asynchronous, conflated customer
 * display refresh showing the itemised basket. The owning session decides when mutations are
 * allowed: a basket frozen by in-flight money movement, already consumed by settlement, or owned by
 * an ended session rejects them with {@code IllegalStateException}. Call {@link #clear()} after
 * settlement to start another basket in the same session.
 *
 * <pre>{@code
...
public final class SessionBasket {
   ...
   * rules (in-flight checks, atomicity, display refresh) and produces snapshots under its lock.
   */
  interface Host {
