...
/**
 * The wire bodies of the terminal's device and admin operations — SERVICE/DEVICE-class nexo
 * messages that carry no session reference. Owned by {@link Terminal}; argument validation and
 * lifecycle guards stay on the public methods.
 */
final class TerminalServices {
  ...
  DiagnosisResult diagnose() {
    SaleToPOIRequest request =
        SaleToPOIRequest.builder()
            .messageHeader(factory.header(MessageClassType.SERVICE, MessageCategoryType.DIAGNOSIS))
            .diagnosisRequest(DiagnosisRequest.builder().build())
            .build();
    SaleToPOIResponse response = exchange.sendExpectingBody(MessageCategoryType.DIAGNOSIS, request);
  ...
  ReconciliationResult getTotals() {
    SaleToPOIRequest request =
        SaleToPOIRequest.builder()
            .messageHeader(factory.header(MessageClassType.SERVICE, MessageCategoryType.GET_TOTALS))
            .getTotalsRequest(
                GetTotalsRequest.builder()
                    .totalFilter(
                        TotalFilter.builder()
    ...
    SaleToPOIResponse response =
        exchange.sendExpectingBody(MessageCategoryType.GET_TOTALS, request);
    ...
    return new ReconciliationResult(
        body.getPoiReconciliationID(),
        body.getTransactionTotals() == null ? null : Arrays.asList(body.getTransactionTotals()));
  }
  ...
  ReconciliationResult reconcile() {
    SaleToPOIRequest request =
        SaleToPOIRequest.builder()
            .messageHeader(
                factory.header(MessageClassType.SERVICE, MessageCategoryType.RECONCILIATION))
            ...
            .build();
    SaleToPOIResponse response =
        exchange.sendExpectingBody(MessageCategoryType.RECONCILIATION, request);
  ...
  Void print(PrintPayload payload) {
    ...
    if (payload.getFormat() == PrintPayload.Format.TEXT) {
      content
          ...
          .outputText(new OutputText[] {OutputText.builder().text(payload.getContent()).build()});
    } else {
      content.outputFormat(OutputFormatEnum.XHTML).outputXHTML(payload.getContent());
    }
    SaleToPOIRequest request =
        SaleToPOIRequest.builder()
            .messageHeader(factory.header(MessageClassType.DEVICE, MessageCategoryType.PRINT))
            .printRequest(
                PrintRequest.builder()
                    ...
                    .build())
            .build();
    SaleToPOIResponse response = exchange.sendExpectingBody(MessageCategoryType.PRINT, request);
  ...
  private Void sendSound(SoundActionEnum action, SoundContent content, Integer volumePercent) {
    SoundRequest.Builder soundRequest =
        SoundRequest.builder().soundAction(action).responseMode(ResponseModeEnum.IMMEDIATE);
    if (content != null) {
    ...
    if (response != null && response.getSoundResponse() != null) {
      exchange.requireSuccess(MessageCategoryType.SOUND, response.getSoundResponse().getResponse());
    }
    return null;
  }
}
