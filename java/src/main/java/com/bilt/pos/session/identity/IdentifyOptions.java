...
public final class IdentifyOptions {
  ...
  private IdentifyOptions(Builder builder) {
    ...
    this.allowedLoyaltyBrands =
        Collections.unmodifiableList(new ArrayList<>(builder.allowedLoyaltyBrands));
  ...
  /**
   * Whether the lookup fails when no member is found ({@code LoyaltyHandling=Required}) or
   * continues without one ({@code Proposed}). Default {@code true}.
   */
