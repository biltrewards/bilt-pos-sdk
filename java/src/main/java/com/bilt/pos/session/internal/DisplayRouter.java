 ...
 *
 * <p>When an external display client is configured, {@code Display} and {@code Input} messages are
 * routed to it; everything else — payments, loyalty, stored value, card acquisition, and notably
 * {@code PIN} (the encrypted PIN block must come from the secure PIN pad) — goes to the terminal.
 */
