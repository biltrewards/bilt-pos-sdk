...
/**
 * Settlement-time stored value fulfillment for a referenced sale line. The card is loaded with the
 * line's original, pre-discount value. Register discounts reduce only the amount charged to the
 * customer, so a fully discounted line still loads the card's full face value.
 */
