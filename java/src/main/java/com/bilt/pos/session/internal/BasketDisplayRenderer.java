...
/**
 * Default {@link DisplayRenderer}: renders the basket as the standard itemised virtual receipt
 * ({@code receipt.xslt} layout).
 */
public final class BasketDisplayRenderer implements DisplayRenderer {
  ...
  public DisplayPayload render(Basket basket, DisplayContext context) {
    ...
    receipt.setSubtotal(DisplayPayloadHelper.labeledAmount("Subtotal", currency, subtotal));
    if (basket.getPointDiscountTotal().signum() != 0) {
      AdjustmentsType adjustments = new AdjustmentsType();
      adjustments
          .getAdjustmentItem()
          .add(
              DisplayPayloadHelper.labeledAmount(
    ...
    if (basket.getTaxTotal().signum() != 0) {
      ...
      tax.setTaxTotal(DisplayPayloadHelper.labeledAmount("Tax", currency, basket.getTaxTotal()));
      receipt.setTax(tax);
    }
    receipt.setTotal(DisplayPayloadHelper.labeledAmount("Total", currency, total));
  ...
  /**
   * The session currency is an ISO 4217 code by contract, but the receipt layout prints {@code
   * MoneyType.currency} verbatim next to every amount — customers should see {@code $79.99}, not
   * {@code USD 79.99}. Codes without a known symbol fall back to the code itself.
   */
