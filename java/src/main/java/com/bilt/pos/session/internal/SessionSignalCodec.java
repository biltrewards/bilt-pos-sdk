...
/**
 * Encodes the Bilt session lifecycle signals carried in the {@code ServiceIdentification} field of
 * a Nexo {@code AdminRequest} — the spec allows "a name or CSV path", so the signal is a versioned
 * CSV:
 *
 * <pre>
 ...
 * </pre>
 *
 * <p>Start announces the session; End tells the terminal to discard the session-scoped data it
 * accumulated. This format is a contract with the terminal firmware — change it only in lockstep,
 * bumping the version.
 */
