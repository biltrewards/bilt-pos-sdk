...
/**
 * A terminal's device and admin operations — diagnostics, totals, reconciliation, printing, sound —
 * outside any session.
 *
 ...
 * send nothing until {@code execute()}, {@code executeSync()}, {@code get()}, or {@code
 * getOrNull()} is invoked. Asynchronous executions run one at a time on the terminal's own
 * operation thread and deliver their handlers through the configured {@link
 * Builder#callbackExecutor(Executor) callbackExecutor}, with the same semantics as the session
 * builders.
 *
 * <pre>{@code
...
public final class Terminal implements AutoCloseable {
  ...
  private Terminal(Builder builder) {
    ...
    NexoMessageFactory factory =
        new NexoMessageFactory(builder.saleId, builder.poiId, builder.storeLocation);
    NexoExchange exchange = new NexoExchange(new DisplayRouter(builder.client, null), factory);
  ...
  /**
   * Queries the terminal's running totals since the last reconciliation (Nexo {@code GetTotals}) —
   * lighter than {@link #reconcile()}, which closes the period. Filtered by this terminal's {@code
   * SaleID} and, when configured, the {@link Builder#storeLocation(String) storeLocation} as {@code
   * TotalsGroupID}.
   */
  ...
  public SessionResult<Void> playSound(String soundReferenceId, Integer volumePercent) {
    ...
    return operation("playSound", () -> services.playSound(soundReferenceId, volumePercent));
  }
  ...
  /**
   * Stops accepting operations. Sends nothing — the terminal holds no state for this object, so
   * there is nothing to end — and is idempotent. Operations executed after this fail with {@link
   * SessionErrorCode#INVALID_STATE} through their normal {@code onError}/{@code onComplete}
   * delivery.
   */
  @Override
  public void close() {
  ...
  public static final class Builder {
    ...
    /**
     * Store location identifier: the {@code TotalsGroupID} that {@link Terminal#getTotals()}
     * filters by. Optional.
     */
    ...
    /**
     * Validates the configuration and returns the terminal. Sends nothing — there is no session
     * bracket to announce.
     *
