...
/**
 * Receiver for batched basket mutations via {@code session.basket().mutate(...)}: all changes are
 * applied atomically and followed by a single display update.
 *
 * <pre>{@code
 ...
 * }</pre>
 *
 * <p>SKU-addressed mutations prefer a unique sale line when a SKU is used by multiple item types.
 * They reject ambiguous SKUs rather than choosing among multiple sale lines, or among multiple
 * non-sale lines when no sale exists; use itemId to address those lines.
 */
