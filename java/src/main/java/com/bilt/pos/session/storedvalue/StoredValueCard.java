...
/**
 * Identifies a stored value (gift) card for stored value operations and split-tender payment.
 *
 * <pre>{@code
...
public final class StoredValueCard {
  ...
  public static StoredValueCard number(String cardNumber) {
    ...
    return new StoredValueCard(
        cardNumber,
        IdentificationTypeEnum.PAN,
        EntryModeType.KEYED,
  ...
  public static StoredValueCard scanned(String barcode) {
    ...
    return new StoredValueCard(
        barcode,
        IdentificationTypeEnum.BAR_CODE,
        EntryModeType.SCANNED,
        StoredValueAccountTypeEnum.GIFT_CARD,
        null,
        null);
  }
  ...
  /**
   * The terminal prompts the customer to swipe the card ({@code MagStripe}; no card number is
   * sent).
   */
  ...
  public StoredValueCard withProvider(String provider) {
    return new StoredValueCard(
        storedValueId, identificationType, entryMode, accountType, provider, expiryDate);
  }
