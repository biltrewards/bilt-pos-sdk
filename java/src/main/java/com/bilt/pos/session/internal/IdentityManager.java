...
public final class IdentityManager {
  ...
  public IdentifyResult identifyPrompted(
    ...
    CardAcquisitionTransaction.Builder transaction =
        CardAcquisitionTransaction.builder()
            .loyaltyHandling(
                options.isRequireMember()
                    ? LoyaltyHandlingEnum.REQUIRED
                    : LoyaltyHandlingEnum.PROPOSED);
    ...
    if (!options.getAllowedLoyaltyBrands().isEmpty()) {
      transaction.allowedLoyaltyBrand(options.getAllowedLoyaltyBrands().toArray(new String[0]));
    }
    ...
    IdentifyStatus lookupStatus =
        lookupStatus(MessageCategoryType.CARD_ACQUISITION, body.getResponse());
    if (lookupStatus != IdentifyStatus.FOUND) {
      return IdentifyResult.withoutMember(lookupStatus);
    }
    LoyaltyAccount account =
        body.getLoyaltyAccount() != null && body.getLoyaltyAccount().length > 0
            ? body.getLoyaltyAccount()[0]
            : null;
  ...
  public IdentifyResult identifyByIdentifier(MemberIdentifier identifier) {
    SaleToPOIRequest request =
        SaleToPOIRequest.builder()
            .messageHeader(
                exchange
                    .factory()
                    .header(MessageClassType.SERVICE, MessageCategoryType.BALANCE_INQUIRY))
            .balanceInquiryRequest(
                BalanceInquiryRequest.builder()
                    .loyaltyAccountReq(
    ...
    SaleToPOIResponse response =
        exchange.sendExpectingBody(MessageCategoryType.BALANCE_INQUIRY, request);
    BalanceInquiryResponse body = response.getBalanceInquiryResponse();
    if (body == null || body.getResponse() == null) {
      throw Wire.missing("BalanceInquiryResponse");
    }
    IdentifyStatus lookupStatus =
        lookupStatus(MessageCategoryType.BALANCE_INQUIRY, body.getResponse());
    if (lookupStatus != IdentifyStatus.FOUND) {
      return IdentifyResult.withoutMember(lookupStatus);
    }
    LoyaltyAccount account =
        body.getLoyaltyAccountStatus() == null
            ? null
            : body.getLoyaltyAccountStatus().getLoyaltyAccount();
    if (account == null || account.getLoyaltyAccountID() == null) {
  ...
  public CardAcquisitionResult acquireCard(
    ...
    CardData card =
        body.getPaymentInstrumentData() == null
            ? null
            : body.getPaymentInstrumentData().getCardData();
    if (card == null) {
    ...
    return CardAcquisitionResult.builder()
        ...
        .rawPan(card.getSensitiveCardData() == null ? null : card.getSensitiveCardData().getPan())
        .expiryDate(
            card.getSensitiveCardData() == null
                ? null
                : card.getSensitiveCardData().getExpiryDate())
        .paymentBrand(card.getPaymentBrand())
        .entryMode(
            card.getEntryMode() != null && card.getEntryMode().length > 0
                ? EntryMode.fromWire(card.getEntryMode()[0])
                : null)
        .cardToken(card.getPaymentToken() == null ? null : card.getPaymentToken().getTokenValue())
   ...
   * customer-cancelled become an {@link IdentifyStatus}; anything else failing throws.
   */
  ...
  /**
   * Additional card data may be Base64 JSON or URL-encoded pairs; fall back to the raw value under
   * {@code "raw"}.
   */
