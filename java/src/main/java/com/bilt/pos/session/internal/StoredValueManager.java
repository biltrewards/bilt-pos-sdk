...
/**
 * Stored value (gift card) lifecycle operations: activate, load, unload, reserve, reverse,
 * duplicate ({@code StoredValueRequest}) and balance inquiry ({@code BalanceInquiry} with a {@code
 * PaymentAccountReq}).
 */
public final class StoredValueManager {
  ...
  public SaleToPOIRequest operationRequest(
      StoredValueTransactionTypeEnum type,
      StoredValueCard card,
      BigDecimal amount,
      String originalPoiTxnId,
    ...
    if (originalPoiTxnId != null) {
      data.originalPOITransaction(Wire.originalTransaction(originalPoiTxnId, originalPoiTimestamp));
    }
  ...
  public StoredValueOperationResult operationResult(
      StoredValueTransactionTypeEnum type, BigDecimal amount, SaleToPOIResponse response) {
    ...
    StoredValueResult result =
        body.getStoredValueResult() != null && body.getStoredValueResult().length > 0
            ? body.getStoredValueResult()[0]
            : null;
    TransactionIdentificationType poiTxn =
        body.getPoiData() == null ? null : body.getPoiData().getPoiTransactionID();
    StoredValueOperationResult.Builder out =
        StoredValueOperationResult.builder()
    ...
    if (result != null) {
      ...
      if (result.getStoredValueAccountStatus() != null
        ...
        out.currentBalance(Wire.money(result.getStoredValueAccountStatus().getCurrentBalance()));
      }
  ...
  public StoredValueBalance balance(StoredValueCard card) {
    ...
    return new StoredValueBalance(
        ...
        body.getPaymentAccountStatus().getCurrency() != null
            ? body.getPaymentAccountStatus().getCurrency()
            : currency);
  }
  ...
  public static StoredValueAccountID accountId(StoredValueCard card) {
    StoredValueAccountID.Builder account =
