...
public final class SettlementRecovery {
  ...
  /**
   * Retry only the failed step, preserving every earlier committed step. When TransactionStatus
   * could not resolve an indeterminate terminal request, retry checks its status again and never
  ...
  /**
   * Skip the failed step. Valid only when all basket obligations remain satisfiable; the final card
   * tender and stored-value fulfillment cannot skip.
   */
  ...
  /**
   * Records a register-managed tender and continues after the failed card step. The payment amount
   * must exactly equal the outstanding amount, and the failure outcome must be definitive. The SDK
   * attempts to resolve an indeterminate original request before asking the register for a recovery
   * decision; external replacement remains invalid unless resolution is definitive, preventing it
   * from replacing a committed card payment.
   */
  ...
  /**
   * Stop without further status recovery, retry, or unwind and transfer responsibility for all
   * committed movements to the register. Automatic TransactionStatus recovery may already have
   * occurred before {@code onError}. The basket remains usable and SDK recovery guards are
   * released, so a later settlement can duplicate those movements unless the register reconciles
   * them.
   */
