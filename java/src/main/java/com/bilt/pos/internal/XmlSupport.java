...
/**
 * Jackson-based XML marshalling for the generated jakarta.xml.bind-annotated models. Replaces the
 * JAXB runtime, which does not run on Android; Jackson reads the same annotations via {@link
 * JakartaXmlBindAnnotationIntrospector}, so the generated model classes stay untouched.
 *
...
public final class XmlSupport {
  ...
  /**
   * Builds an {@link XmlMapper} configured to mirror the previous JAXB marshalling behavior:
   * jakarta annotations drive names/attributes/order, nulls are omitted, empty beans marshal as
   * empty elements ({@code <confirmation/>}), and unknown elements are ignored on read.
   */
  ...
  /**
   * Normalizes marshalled XML to the single-default-namespace shape the terminals expect (and the
   * JAXB-based helpers produced): all namespace prefixes and declarations are stripped, the given
   * default namespace is declared on the root element, and the XML declaration is prepended.
   *
  ...
  public static String withDefaultNamespace(String xml, String rootElement, String namespace) {
    ...
    xml =
        xml.replaceFirst(
            "<" + rootElement + "(?=[ />])", "<" + rootElement + " xmlns=\"" + namespace + "\"");
