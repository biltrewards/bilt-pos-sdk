...
/**
 * Utility class for serializing and deserializing {@link ReceiptType} objects to/from XML and
 * Base64-encoded XML strings.
 *
 * <p>Receipt XML payloads conform to the {@code urn:bilt:receipt:v1} schema and are carried inside
 * {@code OutputContent.OutputXHTML} fields of Nexo {@code PaymentReceipt} objects.
 *
 * <p>Example usage:
 *
 * <pre>{@code
...
public final class ReceiptHelper {
   ...
   *
   * <p>Some terminals put the receipt XML into {@code OutputXHTML} raw, skipping the Base64
   * encoding the schema calls for. A payload opening with {@code '<'} cannot be Base64 ({@code '<'}
   * is outside the alphabet), so it is unambiguously the raw form and parsed as XML directly.
   *
   * @param base64 the Base64-encoded (or raw) XML string, as found in {@code OutputXHTML}
   * @return the deserialized receipt
   * @throws JAXBException if deserialization fails
   */
   ...
   *
   * <p>The schema stores HTML as {@code xs:base64Binary}, so JAXB represents it as {@code byte[]}.
   * This convenience method decodes it to a UTF-8 string.
   *
