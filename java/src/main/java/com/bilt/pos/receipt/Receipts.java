 ...
 *
 * <p>After a payment, reversal, loyalty, or balance inquiry, the terminal returns receipts inside
 * {@code PaymentReceipt[]} with Base64-encoded XML in {@code OutputContent.OutputXHTML}. This class
 * locates the customer and merchant receipt entries, decodes them, and provides typed access to the
 * receipt content.
 *
 * <p>Example usage:
 *
...
public final class Receipts {
   ...
   *
   * <p>Searches for {@code PaymentReceipt} entries across payment, reversal, loyalty, and balance
   * inquiry responses.
   *
   * @param response the Nexo terminal API response
   * @return a {@link Receipts} instance (receipts may be absent if the response does not contain
   *     them)
   ...
   * @throws JAXBException if receipt XML parsing fails
   */
  public static Receipts fromPaymentReceipts(PaymentReceipt[] paymentReceipts)
