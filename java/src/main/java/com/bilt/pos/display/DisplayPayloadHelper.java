...
/**
 * Utility class for serializing {@link DisplayPayload} and {@link InputPayload} objects to
 * Base64-encoded XML strings suitable for use in {@code OutputXHTML} fields.
 *
 * <p>Example usage:
 *
 * <pre>{@code
...
public final class DisplayPayloadHelper {
   ...
   *
   * <p>The output uses a single default namespace declaration ({@code xmlns="urn:bilt:input:v1"})
   ...
   *
   * <p>Parsing is namespace-agnostic: both the normalized XML format (all elements in
   * urn:bilt:input:v1) produced by {@link #toXml(InputPayload)} and the raw JAXB format with
   * separate namespaces are accepted.
   *
   * @param xml the XML string to deserialize
   ...
   * @return a new display payload configured for QR code
   */
  public static DisplayPayload qrCode(
      String layout, String data, String headerText, String callToAction) {
    DisplayPayload payload = new DisplayPayload();
    payload.setVersion("1.0");
    payload.setLayout(layout);
    QrCodeType qrCode = new QrCodeType();
  ...
  /**
   * Creates a display-only input payload for use with nexo native input commands (DigitString,
   * DecimalString, TextString, GetMenuEntry).
   *
   * <p>Selection behavior for GetMenuEntry (single vs multi-select) is controlled by {@code
   * InputData.minLength/maxLength} in the nexo request, not the XML payload.
   *
   ...
   *
   * <p>For use with nexo native input commands (DigitString, DecimalString, TextString,
   * GetMenuEntry).
   *
   * @param title the title/prompt to display
   ...
   *
   * <p>The value is formatted with 2 decimal places to preserve cents precision (e.g., 107.00 stays
   * as "107.00", not "107.0").
   *
   * @param currency the currency symbol or code (e.g., "$" or "USD")
   * @param value the monetary value
   ...
   */
  public static LabeledAmountType labeledAmount(
      String description, String currency, BigDecimal value) {
  ...
  public static LabeledAmountType labeledAmount(String description, String currency, double value) {
    ...
    return labeledAmount(
        description, currency, new BigDecimal(String.format(java.util.Locale.US, "%.2f", value)));
  }
   ...
   * @return a new LineItemType instance
   */
  public static LineItemType productItem(
      String description,
      BigDecimal quantity,
      String currency,
