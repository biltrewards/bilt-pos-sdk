...
- `com.bilt.pos.platform` — the SDK's first contracts for talking to Bilt cloud, shared by the upcoming member resolution, session registration, retail media and other widgets. `BiltCredentials.clientCredentials(clientId, clientSecret)` holds an OAuth 2.0 client-credentials pair with the secret kept out of `toString()`, logs and exceptions; `BiltEnvironment.PRODUCTION`/`STAGING` name the token endpoint (the `ENTERPRISE-POS` Keycloak realm) and the POS API gateway (`api.bilt.com/biltpos`, `api.staging.bilt.dev/biltpos`) per deployment and `BiltEnvironment.custom(tokenEndpoint, apiBaseUrl)` targets a mock server; `BiltPlatformClient` executes transport-neutral `PlatformRequest`s into `PlatformResponse`s, synchronously or as a `CompletableFuture`. `OkHttpPlatformClient` implements it over OkHttp: it acquires a token with the client-credentials grant, caches it, refreshes it in the background before expiry (configurable `refreshSkew`, default 60 s), retries a request once after a `401`, and shares one token request among concurrent callers. Token endpoint failures surface as `PlatformAuthException`. Session builders do not take credentials yet; that wiring lands with the shopper session and widget work.
