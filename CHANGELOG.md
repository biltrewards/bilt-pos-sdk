...
- `updateDisplay(Basket)`/`updateDisplay(DisplayPayload)` and `abort()` are now lazy `SessionResult<Void>`s like every other terminal operation — nothing is sent until `execute()` (returns immediately) or `executeSync()` (blocks) is invoked (migration: append `.executeSync()` for the old semantics). `updateDisplay` runs on the session's operation lane and delivers failures through its own per-call `onError` instead of swallowing them into the JUL log; after `end()` it fails with `INVALID_STATE` rather than skipping quietly. `abort()` (on both session types) is deliberately *unordered* — like `updateInputDisplay`, it exists to overlap the in-flight operation occupying the operation lane, so `abort().execute()` overtakes a parked payment instead of queueing behind the very thing it cancels.
- The automatic customer-display refresh (`autoDisplay`, default on) is now **asynchronous and conflated**: a basket (or refund cart) mutation is pure local compute — it returns the updated snapshot immediately and enqueues the display push on the session's operation lane, so a register can ring items from its UI thread without paying a terminal roundtrip per tap. Pushes conflate to the newest snapshot (at most one queued plus one in flight — a fast ring-up skips the display straight to the current state, and a `pay()` executed after ring-up waits at most one roundtrip), stay ordered against payments (a push can never land mid-payment and overwrite the terminal's payment screen), and a push that outlives the live basket — the payment completed, a void sealed the cart, or the session ended before it ran — sends nothing, so a settled screen is never overwritten by a stale cart. Failures remain best-effort — logged, never interrupting the checkout — and additionally report through the new `onBackgroundError` handler.
- `pay()` no longer verifies anything at creation — the preconditions (payable state, non-empty basket, positive grand total) are checked when the returned `PaymentFlow` executes, and a violation is delivered to the flow's `onError` handler as `INVALID_STATE` (the blocking accessors throw the `SessionException`) instead of `pay()` throwing `IllegalStateException` before any handler could be registered. Registers driving the checkout through handlers no longer need a try/catch around `pay()`; ones relying on the eager throw should move that handling to `onError` or catch `SessionException` at execution.
- The device and admin operations — `diagnose()`, `getTotals()`, `reconcile()`, `print()`, `playSound()`/`stopSound()` — moved off `CheckoutSession` onto the new `Terminal` (migration: `session.diagnose()` → `session.terminal().diagnose()`, etc.). They are SERVICE/DEVICE-class nexo messages with no session reference on the wire, and the move makes them behave like it: they work with no session at all (no `Start` bracket needed — a connectivity ping before the first checkout no longer announces a session), keep working after `end()`, and no longer queue behind an in-flight payment (the session's `Terminal` has its own operation thread and exchange, so `session.abort()` does not target them). `getTransactionStatus()` stays on the sessions — its register use is session-contextual.
- `execute()` is now **asynchronous** on `SessionResult`, `PaymentFlow`, and `ReversalFlow`: it submits the operation to the session's operation executor — a single lazily-created thread per session, so operations run one at a time in submission order — and returns immediately, delivering the outcome through the registered handlers. The previous blocking behavior is now spelled **`executeSync()`**; `get()`/`getOrNull()`/`isSuccess()` are unchanged (and wait for an in-flight `execute()` to settle). Every execution — sync included — funnels through the session's operation queue, so a blocking call issued while an asynchronous one is in flight waits its turn instead of racing it; a sync call made from the operation thread itself (an operation body or direct-delivered handler invoking another operation) runs inline. The one deliberate exception is `updateInputDisplay`, which exists to overlap the in-flight input it amends. Migration: a mechanical `execute()` → `executeSync()` keeps the old semantics everywhere.
- Callback delivery for asynchronous operations is configurable: `callbackExecutor(Executor)` on the `CheckoutSession`/`ReversalSession` builders sets the session-wide default (e.g. Android's main-thread executor), `callbackOn(executor)` overrides per call/flow. For `PaymentFlow`/`ReversalFlow`, every handler — step handlers included — is delivered on the callback executor with the flow thread awaiting the answer, so the value-returning handlers steer the sequence exactly as if inline while physically running on the integrator's thread (keep them quick, and never block a callback thread on the flow's `get()`). Without a callback executor, handlers run directly on the session's operation thread and must be fast and non-blocking. Handlers may invoke further session operations synchronously — an operation started from inside a handler runs inline, as part of the operation being handled. Do not block the callback executor's thread on a session call *outside* a handler while operations are in flight (an in-flight operation may need that thread for its handlers); from such a thread — teardown included — use `execute()` with `onComplete` rather than `executeSync()`/`close()`.
...
- `onBackgroundError(Consumer<SessionError>)` on the `CheckoutSession`/`ReversalSession` builders: a handler for failures of work the session performs on its own behalf, with no result object to report through. Today that is the automatic display push after a basket/refund-cart mutation; the reserved reactive `onBasketUpdated` channel will report here too. Manual `updateDisplay(...)` failures do **not** route here — they have their own per-call `onError`. Background failures never interrupt the checkout, are always logged, and are delivered through the `callbackExecutor` when one is configured (directly on the failing thread otherwise).
- `Terminal` — the session-less home of the device and admin operations (`com.bilt.pos.session`). `Terminal.builder()` takes `client`/`saleId`/`poiId` (plus optional `storeLocation`, the `TotalsGroupID` that `getTotals()` filters by, and `callbackExecutor` with the session builders' semantics) and `build()` sends nothing — there is no bracket. Operations are lazy `SessionResult`s with the full async `execute()` machinery. `close()` sends nothing and only stops the object accepting operations. `CheckoutSession.terminal()` and `ReversalSession.terminal()` return a cached `Terminal` built from the session's configuration, keeping the operations one call away mid-checkout.
