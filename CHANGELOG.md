...
- `TerminalClient` (`com.bilt.pos.nexo.client`) — the transport seam between the session layer and the terminal: `request(NexoTerminalAPI)` and `request(NexoTerminalAPI, Duration)`, which is all a session ever calls. `BiltNexoTerminalClient` implements it (the class and its builder are unchanged), and `TerminalShopperSession.Builder.client(..)`/`externalDisplayClient(..)`, `Terminal.Builder.client(..)`, and both `getClient()` accessors are now typed against the interface, so a register can plug in another transport — a cloud-relayed implementation is planned — or an in-memory fake in tests. Existing code that passes a `BiltNexoTerminalClient` compiles unchanged; code that assigned `getClient()` to a `BiltNexoTerminalClient` variable needs the declared type widened to `TerminalClient`.
