...
- `CheckoutSession.pay()` is replaced by `CheckoutSession.settle()`. `SettlementFlow` now handles sale, return, register-credit, and stored-value-load lines, refund allocations, exchanges, and refund-only settlements while retaining the total-returning callbacks for rebate, point, and gift-card charge steps.
...
- First-class stored value purchases: `BasketItem.storedValueLoad(reference, sku, description, faceValue)` adds the commercial line, while `SettlementOptions.addFulfillment(StoredValueLoad.activate/reload(reference, card))` supplies the terminal instruction. Settlement funds the basket before loading the card, rolls funding back when fulfillment fails, records each load in `SettlementResult` / `OriginalSaleRecord`, and reverses loads before funding during a whole-sale void.
