...
- `CheckoutSession` is split into two interfaces and renamed. `ShopperSession` is one shopper's visit at a lane, with or without a terminal: lifecycle (`end()`, `close()`), `basket()`, `getMember()`, and the `getSessionId()`/`getSaleId()`/`getCurrency()`/`getStoreLocation()` accessors — no settlement and no terminal operations. `TerminalShopperSession extends ShopperSession` carries everything that needs a terminal: `identifyMember`, `acquireCard`, the `request*` prompts, the stored value operations, `settle`, `refund*`, `voidTransaction*`, `updateDisplay*`, `updateInputDisplay`, `abort`, `forceEnd`, `getTransactionStatus`, `terminal()`, `getClient()`, and the new `getPoiId()`. Migration: `CheckoutSession` → `TerminalShopperSession`, `CheckoutSession.builder()` → `TerminalShopperSession.builder()` (same builder surface; `start()` yields a `SessionResult<TerminalShopperSession>`), `CheckoutSession.Builder` → `TerminalShopperSession.Builder`. There is no deprecated alias. Entries below that name `CheckoutSession` describe what is now `TerminalShopperSession`.
...
- `ShopperSession.context()` — a `SessionContext` on every shopper session carrying the `CheckoutPhase` (`SCANNING`, `MEMBER_IDENTIFIED`, `TENDERING`, `COMPLETE`), free-form string attributes (`attribute(key, value)`, `removeAttribute(key)`, `attributes()`), and the read-only lane identifiers (`saleId()`, `currency()`, `storeLocation()`, `poiId()`); `snapshot()` returns an immutable `SessionContextSnapshot` value. Pure local compute, safe from any thread, refused after `end()`. Both builders take `phase(CheckoutPhase)` and repeatable `attribute(key, value)` to pre-seed it. A `TerminalShopperSession` moves the phase itself: `TENDERING` when `settle()` begins executing, `COMPLETE` when it succeeds, back to the phase the checkout had when a settlement fails or is aborted, and `SCANNING` on `basket().clear()`; `MEMBER_IDENTIFIED` is never set automatically.
- `SessionBasket.replace(Basket)` and `replace(List<BasketItem>)` — a third basket updater, for a POS that owns its own cart: hand the SDK the whole cart after every change instead of translating each edit into incremental calls. The session basket becomes equal to the snapshot's lines, discounts, taxes and tax total; lines are paired with the current basket by `reference`, else by SKU and item type, and paired lines keep their `itemId` so register references stay stable while new lines get new ids. Ambiguous snapshots (two unreferenced lines with the same SKU and type, or a repeated reference) are rejected with `IllegalArgumentException` and leave the basket untouched. On a terminal session whose basket has settled, `replace()` starts a fresh cart with a new cart id the way `clear()` does instead of failing. A snapshot equal to the current basket is an empty change and pushes nothing to the customer display.
- `BasketChange` (`com.bilt.pos.session.basket`) — the immutable value every basket updater ends in: `previous()` and `current()` snapshots, a `source()` of `INCREMENTAL`, `BATCH`, `REPLACE` or `CLEAR`, and the diff between the two (`added()`, `removed()`, `quantityChanged()`, `priceChanged()`, `discountsChanged()`, `taxChanged()` as before/after `LineChange`s, `taxTotalChanged()`, `isEmpty()`), computed by `BasketChange.between(previous, current, source)` under the same pairing rules. A `mutate(..)` batch is one change, `clear()` one change with source `CLEAR`, and a change that leaves the basket as it was is not notified. `Basket.getCounterpart(BasketLineItem|BasketItem)` exposes the pairing rule.
- `ShopperSession.builder()` — a local shopper session with no terminal attached: `saleId`, `currency`, `storeLocation`, `callbackExecutor`, `onBackgroundError`, and `start()` returning the session directly (there is nothing to acknowledge, so no lazy `SessionResult`). It rings baskets through `basket()` and ends with `end()`, for lanes without a terminal or registers that only need the basket model and member state.
- `getSaleId()` on every shopper session and `getPoiId()` on terminal sessions.
- `com.bilt.pos.widget` — the widget-to-register seam for on-screen content (RET-6769). `Surface` is the two-verb contract a register implements (`show(Rendering, ActionSink)` replaces what is on screen, `clear()` is idempotent); `Rendering` is an immutable, PII-free structured creative (`MediaSpec` image/video/HTML, headline and body, primary and secondary `Cta` with an `Action` and opaque token, TTL, tracking beacons); `ActionSink` is where a surface reports taps, viewability, dismissal and video completion, validating every CTA token against the rendering it handed out. `WebSurface` is a platform-neutral abstract base for registers that embed a browser control: subclasses wire `loadUrl`/`evaluateJavascript` and forward the page's messages to `onBridgeMessage`; the base loads the hosted renderer page lazily, gates on its `ready` signal, pushes renderings as `window.BiltMedia.receive({...})`, allow-lists inbound message types, checks `creativeId` and token against the current rendering, and reports anything malformed or foreign through `onBridgeError` instead of throwing into the platform callback. Both bridge directions are specified by `bridge-messages.schema.json`, shipped on the classpath under `com/bilt/pos/widget/`. The package Javadoc describes the five surface tiers; adapters live in platform modules.
...
- First-class stored value purchases: any referenced `BasketItem.sale(...)` can be paired with `SettlementOptions.addFulfillment(StoredValueLoad.activate/reload(reference, card))`. Settlement uses the sale line's original total as the load amount, loads the card before collecting tender, and reverses completed loads when a later failure is aborted. Failed loads prevent charging; charge retries retain completed loads without repeating them. Each load is recorded in `SettlementResult` / `OriginalSaleRecord`, and whole-sale voids reverse loads before funding.
...
- Emulator: a **Loyalty Sign-In** button beside Start Checkout runs the terminal's member-identification prompt on demand, so a customer who declined or mistyped at the start of the checkout can sign in again without restarting it. The terminal's answer is rendered as its own card — member, any brand and point balance the terminal reports, and each reward's handle, description, and expiry — and is held until the checkout ends.
- Emulator controls for gift-card purchases and item-level register discounts/credits. Gift-card loads are persisted with completed sales so a later full refund can reverse the load before its funding.
...
- Structured cross-session reversal progress through `SessionError.getReversedMovements()`, identifying each movement that completed before an abort by `ReversalStep` and POI transaction ID.
...
- Emulator: the Sale tab splits into Products and Keypad panes. The keypad rings a typed amount up as a custom, untaxed sale line; tapping a custom line in the basket adopts it, so every keystroke re-prices that line live, and ✓ releases it so the next digits start a new item.
