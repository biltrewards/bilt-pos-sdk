...
- `com.bilt.pos.media` and `com.bilt.pos.media.service` — the generic ad decision contract the retail-media widget will be built on, defined ahead of the platform integration spec (RET-6771; the platform-backed implementation is RET-6784). `AdDecisionService` registers a shopper session from an `AdSessionSnapshot` (sale, store, lane, currency, free-form phase, attributes, member id and `LineSummary` lines — deliberately independent of `CheckoutSession` and the basket types), answers `decide(handle, placement, capabilities, timeout)` with an `Optional<Rendering>` within the timeout and never throws for a miss, validates taps with `validateAction` → `ActionOutcome` (accepted with an `Offer` for `APPLY_OFFER`, accepted without one otherwise, or rejected with a reason), records `AdInteraction`s through `report`, and opens a server-to-SDK event channel through `subscribe(handle, AdEventListener)` returning an `AutoCloseable` `AdEventSubscription`. Supporting value types: `Placement` (a named zone from the platform's placement map), `Offer` (basket or line-item scope, exactly one of amount or percentage, optional expiry), `AdInteraction` (kind, timestamp, optional `Action`), `SurfaceKind`, and `Capabilities` (SDK version, media formats, actions, placements with their surface kind) sent with every decision so the platform only serves what this build can render. `InMemoryAdDecisionService` is the fake for tests and the emulator: scripted renderings per placement (rotating or by rule over the latest snapshot), artificial latency, one-shot failure injection, recorded snapshots, served renderings and reports, capability filtering, token bookkeeping with `offerFor(token, offer)`, and test-injected events to subscribers.
