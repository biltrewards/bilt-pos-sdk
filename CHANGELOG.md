...
- `MemberIdentifier` is removed in favor of `Member` (`com.bilt.pos.session.identity`). `TerminalShopperSession.identifyMember(MemberIdentifier)` becomes `identifyMember(Member pending)` and takes a member pending resolution; migration: `MemberIdentifier.phoneNumber("...")` → `Member.idResolver().phone("...")`, `MemberIdentifier.accountNumber("...")` → `Member.idResolver().accountId("...")`, `.keyedByCashier()` unchanged. `ShopperSession.getMember()` is deprecated in favor of `member()`, which also reports a POS-attached member and one pending resolution; it still returns the resolved member as an `IdentifyResult` (`null` while pending) and will be removed in a later release.
...
- `Member` and `ShopperSession.member(Member)` / `member()` — a POS-provided member on both session types, settable at any time. `Member.id("mbr_...")` attaches a known Bilt member id immediately with no roundtrip; `Member.idResolver().accountId(...)` / `.phone(...)` / `.email(...)` / `.custom(type, value)` (optionally `.keyedByCashier()`) attach a member pending resolution that the session resolves in the background on its operation lane — on a terminal session through the same Nexo `BalanceInquiry` as `identifyMember(Member)` (account id and phone only; email and custom identifiers report `SessionErrorCode.UNSUPPORTED` through `onBackgroundError` and stay pending), on a local session not yet (the platform resolver is a later release; the member stays pending). Whatever member is attached last wins. `member(null)` signs out. `Member` carries `isResolved()`, `memberId()`, `resolver()` (a `MemberIdResolver`: type, value, `customType()`, `keyedByCashier()`), `status()`, and what the terminal reported — `loyaltyBrand()`, `rewards()`, `pointBalance()`; it has value semantics and a `toString()` that masks phone numbers and email addresses. `Reward` gains value semantics too.
- `member(Member)` pre-seed and `onMemberChanged(Consumer<Member>)` on both session builders. The handler is delivered on the callback executor for every change of the member — attached by the POS, found by a terminal prompt or lookup, resolved in the background, or cleared (`null`); the pre-seeded member is initial state and is not announced.
- `SessionErrorCode.UNSUPPORTED` for what the session type or terminal cannot do.
