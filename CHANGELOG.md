...
- `CheckoutSession.pay()` is replaced by `CheckoutSession.settle()`. `SettlementFlow` now handles sale lines, return credit lines, refund allocations, exchanges, and refund-only settlements while retaining the total-returning callbacks for rebate, point, and gift-card charge steps.
...
- `ReversalSession` is removed. Whole prior-sale voids now use `CheckoutSession.voidTransaction(OriginalSaleRecord)` from any open checkout session; item returns and exchanges use credit lines plus `SettlementOptions` refund allocations.
...
- Net settlement for mixed sale/return baskets: `SettlementOptions.builder().settlementOption(SettlementOption.NET)` charges or refunds only the signed difference. `Basket.getRefundAmount(SettlementOption)` exposes the required allocation total for either settlement mode before settlement starts.
...
- Credit lines and refund allocations: `BasketItem.credit(sku, description, quantity, unitPrice)` marks a line that subtracts from the basket. `SettlementOptions.refundAllocations(...)` / `addRefundAllocation(...)` tells `settle()` how the register wants that return value restored across card, stored value, point redemption, rebate, and award movements. Separate-settlement allocations match the credit-line total; net-settlement allocations match the net refund instead. The SDK does not choose tender policy.
