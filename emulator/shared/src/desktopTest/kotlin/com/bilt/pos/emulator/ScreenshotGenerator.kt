...
/**
 * Renders the full emulator UI headlessly ([ImageComposeScene], no window or display needed) in two
 * representative states — a render smoke test that doubles as the screenshot generator: the PNGs
 * land in `build/screenshots`, and the copies committed under `emulator/docs` are refreshed from
 * there when the UI changes.
 */
                ...
                sales =
                    listOf(
                        ...
                        StoredSaleUi(
                            ...
                            totalAmount = "42.50",
                            items = listOf(SaleItemUi("SKU-007", "Coffee Beans 1kg", 1, 4250)),
                            refunded = true,
                            fullyRefunded = true,
                        ),
                        StoredSaleUi(
