...
/**
 * Exercises the public [TlsVerifier.verify] against real TLS handshakes: a local server presents a
 * leaf minted under a test CA with terminal-style synthetic SANs, covering chain validation, SAN
 * extraction, and the hostname-pattern semantics together.
 */
class TlsVerifierTest {

    private val ca =
        HeldCertificate.Builder().certificateAuthority(1).commonName("Bilt Test CA").build()
    ...
    fun failsWhenChainDoesNotReachTheConfiguredCa() {
        val server = serverPresenting("V400m-123.pos.staging.bilt.dev")
        val otherCa =
            HeldCertificate.Builder().certificateAuthority(1).commonName("Unrelated CA").build()

        val status =
            TlsVerifier.verify(
                server.hostName,
                server.port,
                otherCa.certificatePem(),
                "*.pos.staging.bilt.dev",
            )
