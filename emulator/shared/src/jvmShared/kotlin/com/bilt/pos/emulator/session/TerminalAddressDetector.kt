...
/**
 * Best-effort terminal discovery via adb, ported from the Python emulator's `_detect_device_ip`:
 * take the first attached device and read its network address. Every hard failure (no adb anywhere,
 * no device — including running on Android itself, where adb doesn't exist) returns null and the
 * operator types the address by hand. A device WITHOUT a network address is still a detection: a
 * USB-only terminal is reachable through the adb tunnel, which needs no address.
 */
object TerminalAddressDetector {
    ...
    fun detect(): Detection? {
        ...
        val address =
            Adb.runOrNull("-s", serial, "shell", "ip", "-f", "inet", "addr", "show")
                ?.let(::parseGlobalIpv4)
        if (address == null) {
    ...
    /**
     * The first global-scope IPv4 from `ip -f inet addr show` output — loopback and link-local
     * addresses carry other scopes and are skipped.
     */
