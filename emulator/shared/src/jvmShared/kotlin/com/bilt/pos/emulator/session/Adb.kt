...
/**
 * Locates and runs the adb executable for the emulator's device integrations — the address
 * autodetect and the adb tunnel.
 *
 * Resolution prefers PATH: that is the adb the operator's terminal uses, and running a DIFFERENT
 * install (a second SDK copy) would kill their adb server over the version mismatch and leave
 * devices enumerating late. The conventional SDK locations are fallbacks for launches that don't
 * inherit the shell's PATH (a Finder-launched app sees only the system default). Every candidate is
 * probed with `adb version` — which does not start the server — so the resolution log states what
 * actually runs.
 *
 * Logs through JUL under `com.bilt.pos`, so the emulator's Detailed tab carries the whole trail:
 * the resolved executable, every command, and the raw output.
 */
internal object Adb {
    ...
    /**
     * The resolved adb executable: the first candidate whose `adb version` answers — PATH first,
     * then the SDK locations.
     */
    ...
    private fun probe(candidate: String): String? =
        try {
    ...
    /**
     * Runs adb with [args] and returns its combined output. The timeout is generous because the
     * first call may have to start the adb server.
     *
     * @throws IllegalStateException when adb is missing, times out, or exits nonzero — the message
     *   carries adb's own output
     */
    fun run(vararg args: String): String {
        ...
        if (process.exitValue() != 0) {
            ...
            throw IllegalStateException("adb failed (${args.joinToString(" ")}): ${output.trim()}")
        }
        return output
    }
    ...
    /**
     * The attached-device serials, retried once when the call had to start the adb server — USB
     * devices can take a moment to enumerate after that, and an empty first answer would read as
     * "no device".
     */
    fun devices(): List<String> {
        ...
        return parseSerials(retried).also {
            LOGGER.info(
                if (it.isEmpty())
                    "adb devices found none after retry; raw output: ${retried.trim()}"
                else "adb devices: $it"
            )
        }
    }
    ...
    /**
     * The attached-device serials from `adb devices` output. Tolerates tabs or spaces between
     * serial and state, and skips the header, the daemon-start banner, and offline/unauthorized
     * devices.
     */
