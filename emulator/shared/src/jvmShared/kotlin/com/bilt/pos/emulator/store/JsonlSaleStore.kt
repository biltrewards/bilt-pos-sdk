...
/**
 * [SaleStore] backed by an append-only JSONL file: one JSON object per line, each a sale, refund,
 * or void event. Appending never rewrites earlier records, so a crash can at worst tear the final
 * line — reads skip lines that do not parse. Reads fold the event log into the current state.
 */
class JsonlSaleStore(private val file: File) : SaleStore {

    companion object {
        /**
         * The store's file within a platform's data directory, so the two entry points don't each
         * restate the layout.
         */
    ...
    override fun listSales(limit: Int): List<StoredSale> = fold().values.reversed().take(limit)
    ...
    /**
     * Sales by id in recording order, with refunds/voids folded in. Events referencing an unknown
     * sale (foreign or truncated file) are dropped.
     */
    private fun fold(): Map<String, StoredSale> {
        ...
        for (event in readEvents()) {
            when (event) {
                ...
                is SaleEvent.Refund ->
                    sales.computeIfPresent(event.saleId) { _, held ->
    ...
    private fun readEvents(): List<SaleEvent> {
        ...
        return synchronized(this) { file.readLines() }
            .mapNotNull {
                runCatching { json.decodeFromString(SaleEvent.serializer(), it) }.getOrNull()
            }
    }
    ...
    private sealed interface SaleEvent {
        ...
        @Serializable @SerialName("sale") data class Sale(val sale: SaleRecord) : SaleEvent
