...
/**
 * Emulator configuration, mirroring the conventions of the Python nexo emulator and the CLI: values
 * come from environment variables first, then from a `local.properties` found by walking up from
 * the working directory (the same file the Android tooling reads, so one place holds the secrets).
 *
 * Keys:
 ...
 * - `NEXO_CA_CERT` — inline PEM (a .properties value is one line; encode line breaks as literal
...
data class EmulatorConfig(
    ...
    val caPem: String?,
    /**
     * Set when a CA *was* configured but couldn't be loaded (bad NEXO_CA_BUNDLE path, unreadable
     * file) — reported as a TLS failure rather than silently degrading to "no CA configured".
     */
