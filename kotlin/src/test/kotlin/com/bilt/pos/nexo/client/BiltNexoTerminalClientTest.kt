...
class BiltNexoTerminalClientTest {
    ...
    private val responseHeaderJson =
        """"MessageHeader":{"ProtocolVersion":"3.0","MessageClass":"Service","MessageCategory":"Payment","MessageType":"Response","POIID":"TERM-1","SaleID":"POS-1","ServiceID":"txn-001"}"""

    // Minimal valid POIData + SaleData blocks required by PaymentResponse
    private val poiDataJson =
        """"POIData":{"POITransactionID":{"TransactionID":"POI-001","TimeStamp":"2026-04-20T14:30:00Z"}}"""
    private val saleDataJson =
        """"SaleData":{"SaleTransactionID":{"TransactionID":"TX-001","TimeStamp":"2026-04-20T14:30:00Z"}}"""
    ...
    private fun minimalResponseJson() = """{"SaleToPOIResponse":{$responseHeaderJson}}"""

    @BeforeEach
    fun setUp() {
        server = MockWebServer()
        server.start()
        client = BiltNexoTerminalClient(endpoint = server.url("/nexo").toString())
    }
    ...
    private fun paymentRequest(amount: Double = 25.0) =
        NexoTerminalAPI(
            saleToPOIRequest =
                SaleToPOIRequest(
    ...
    fun `request throws on HTTP error`() {
        ...
        val ex =
            assertThrows<BiltNexoClientException> {
    ...
    fun `encrypted client handles unencrypted error response`() {
        ...
        server.enqueue(MockResponse().setBody(failureResponseJson("Encryption not configured")))
