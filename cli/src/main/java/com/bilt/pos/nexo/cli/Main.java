...
public final class Main {
  ...
  /**
   * Strong reference to OkHttp's HTTP/2 frame logger, retained so JUL's (weak-referencing)
   * LogManager cannot garbage-collect the configuration we install in {@link #enablePongLogging()}
   * before OkHttp lazily creates the logger. See that method for details.
   */
  ...
  public static void main(String[] args) {
    ...
    if (encryption && (passphrase == null || keyId == null)) {
      LOG.severe(
          "Encryption is enabled by default. Provide --passphrase and --key-id, or use --no-encryption.");
      System.exit(1);
      return;
    }
  ...
  private static void run(
    ...
    BiltNexoTerminalClient.Builder clientBuilder =
        BiltNexoTerminalClient.builder().endpoint(endpoint);

    if (pingIntervalSeconds != null) {
      Duration pingInterval = Duration.ofMillis((long) (pingIntervalSeconds * 1000));
      LOG.info(
          "Sending keep-alive pings every "
              + pingIntervalSeconds
    ...
    if (cacert != null || hostnamePattern != null) {
      ...
      clientBuilder.trustCertificate(Path.of(cacert)).expectedHostnamePattern(hostnamePattern);
    } else {
      ...
      case "refund":
        if (originalServiceID != null) {
          if (originalTimestamp == null) {
            throw new IllegalArgumentException(
                "--original-timestamp is required for referenced refund requests");
          }
          request =
              buildReferencedRefundRequest(
                  serviceID, originalServiceID, originalTimestamp, amount, currency);
        } else {
      ...
      case "confirmation":
        request =
            buildConfirmationRequest(
                serviceID, prompt != null ? prompt : "Would you like a receipt?");
        break;
      case "signature":
        request = buildSignatureRequest(serviceID, prompt != null ? prompt : "Signature required");
        break;
      case "reversal":
        if (originalServiceID == null) {
          throw new IllegalArgumentException(
              "--original-service-id is required for reversal requests");
        }
        if (originalTimestamp == null) {
          throw new IllegalArgumentException(
              "--original-timestamp is required for reversal requests");
        }
        request =
            buildReversalRequest(serviceID, originalServiceID, originalTimestamp, reversalReason);
        break;
    ...
    NexoTerminalAPI apiRequest = NexoTerminalAPI.builder().saleToPOIRequest(request).build();
  ...
  private static SaleToPOIRequest buildPaymentRequest(
      String serviceID, double amount, String currency) {
            ...
            PaymentRequest.builder()
                ...
                .paymentTransaction(
                    PaymentTransaction.builder()
                        .amountsReq(
                            AmountsReq.builder().currency(currency).requestedAmount(amount).build())
                        .build())
                .build())
        .build();
  }

  private static SaleToPOIRequest buildGiftCardPaymentRequest(
      String serviceID, double amount, String currency) {
    return SaleToPOIRequest.builder()
        .messageHeader(
            MessageHeader.builder()
                        ...
                        .paymentInstrumentData(
                            PaymentInstrumentData.builder()
                                ...
                                .storedValueAccountID(
                                    StoredValueAccountID.builder()
                                        .storedValueAccountType(
                                            StoredValueAccountTypeEnum.GIFT_CARD)
                                        .build())
                                .build())
                        .build())
                .saleData(
  ...
  private static SaleToPOIRequest buildReferencedRefundRequest(
      String serviceID,
      String originalServiceID,
      String originalTimestamp,
      double amount,
                ...
                .paymentTransaction(
                    PaymentTransaction.builder()
                        ...
                        .originalPOITransaction(
                            OriginalPOITransaction.builder()
                                .poiTransactionID(
                                    TransactionIdentificationType.builder()
  ...
  private static SaleToPOIRequest buildUnreferencedRefundRequest(
      String serviceID, double amount, String currency) {
    return SaleToPOIRequest.builder()
        .messageHeader(
            MessageHeader.builder()
  ...
  private static SaleToPOIRequest buildInputRequest(String serviceID, InputPayload inputPayload)
      throws JAXBException {
  ...
  private static SaleToPOIRequest buildReversalRequest(
      String serviceID, String originalServiceID, String originalTimestamp, String reversalReason) {
  ...
  /**
   * Raise the SDK client logger to {@code FINE} so its encrypt/decrypt trace messages are printed.
   * The SDK logs these at {@code FINE}, which the default JUL configuration suppresses, so attach a
   * dedicated console handler at that level rather than relying on the (INFO-level) root handler.
   */
   ...
   *
   * <p>OkHttp exposes no public callback for HTTP/2 pongs, so the only place a received pong is
   * observable is its internal frame logger ({@code okhttp3.internal.http2.Http2}), which logs
   * every frame at {@code FINE} in the form {@code "<< 0x%08x %5d %-13s %s"}. We raise that logger
   * to {@code FINE} but suppress its own output ({@code setUseParentHandlers(false)}) and attach a
   * handler that forwards only inbound {@code PING} frames carrying the {@code ACK} flag — the pong
   * the terminal sends in reply to our ping. Our outbound ping, a server-initiated ping request,
   * and our ACK reply to it are all excluded, so exactly the pongs answering our keep-alive pings
   * are printed.
   */
  ...
  /**
   * Waits for Enter so the operator can verify the terminal display before the demo moves on.
   * Skipped silently when stdin is not interactive (piped input or EOF).
   */
  ...
  private static void runCheckoutSessionDemo(
    ...
    try (com.bilt.pos.session.CheckoutSession session =
      ...
      if (identifyMember) {
        ...
        LOG.info("Skipping member identification (pass --identify to enable); " + "guest checkout");
      }
      ...
      com.bilt.pos.session.basket.Basket basket =
          session
              .basket()
              .addItem(
                  com.bilt.pos.session.basket.BasketItem.builder()
                      .sku("CLI-DEMO-1")
                      .description("Demo Item A")
                      .quantity(1)
                      .unitPrice(half)
                      .build());
      ...
      basket =
          session
              ...
              .addItem(
                  com.bilt.pos.session.basket.BasketItem.builder()
                      .sku("CLI-DEMO-2")
                      .description("Demo Item B")
                      .quantity(1)
                      .unitPrice(BigDecimal.valueOf(amount).subtract(half))
                      .build());
      ...
      com.bilt.pos.session.settlement.SettlementResult checkout =
          session
              .settle()
              .onRebatesRedeemed(
                  rebates -> {
      ...
      session.end().onError(error -> LOG.warning("Session end failed: " + error)).executeSync();
