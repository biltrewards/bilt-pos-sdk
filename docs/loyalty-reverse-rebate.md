...
> **Referenced reversal through the session API:** `TerminalShopperSession.voidTransaction(OriginalSaleRecord)` sends this request when the record carries `rebatePoiTransactionId` / `rebatePoiTransactionTimestamp` (persist both from the original sale's `SettlementResult` — `getRebatePoiTransactionId()`, `getRebatePoiTransactionTimestamp()`); `memberId` on the record populates `LoyaltyData[].LoyaltyAccountID`. See the [integration guide](./checkout-session-integration.md#reversing-a-prior-sale-originalsalerecord).
