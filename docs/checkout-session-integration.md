...
- `SettlementRecovery.external(ExternalPayment)` — after the final card tender fails, record a register-managed tender such as cash and continue. Its amount must exactly equal `failure.getAmountDue()`; cashback transactions cannot use this substitution. For an `INDETERMINATE` failure, TransactionStatus runs before the external tender is recorded. If it recovers a successful original card charge, that `CARD_CHARGE` is delivered through `onCardCharged` / `onMovement`, the external decision is superseded, and `onExternallyPaid` does not run. Collect cash or another external tender only when the failure is `DEFINITIVE`, or be prepared to return it if `onExternallyPaid` does not run.
...
When the failed terminal request may have completed despite a timeout or network error, the SDK resolves its `ServiceID` with TransactionStatus before applying retry, skip, external replacement, or abort. If the terminal reports `IN_PROGRESS`, the SDK briefly polls TransactionStatus a bounded number of times before surfacing an unresolved failure. A recovered successful response is consumed as the step result instead of resending the request. `abandon()` deliberately performs no additional terminal I/O.
