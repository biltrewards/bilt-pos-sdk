...
4. **Stored value fulfillment** (for each supplied fulfillment targeting a referenced sale line) — terminal activates or reloads the purchased card before collecting payment → `onStoredValueLoaded`. A failed load prevents the tender charges; aborting a later charge failure reverses completed loads.
5. **Stored value charge** (if a card was registered and a balance remains) — terminal charges the gift card → `onGiftCardPayment`
6. **Card charge** (if a balance remains) — terminal processes the card for the remaining amount → `onCardCharged`; after a failure, the register may satisfy this final tender externally (for example, with cash) → `onExternallyPaid`
...
For a charge-side error with a known outcome, `onError` receives a `SettlementFailure` before any register-directed recovery or unwind. If a terminal request's outcome is indeterminate, the SDK first resolves its `ServiceID` with TransactionStatus. A recovered success is consumed as the step result and settlement continues without calling `onError`; `NOT_FOUND` or a recovered failure reaches `onError` with `DEFINITIVE` certainty. If TransactionStatus itself cannot resolve the request, `onError` receives an `INDETERMINATE` failure. The failure identifies the step, outstanding amount, and committed movement ledger, and its returned `SettlementRecovery` controls the next action:
...
- `SettlementRecovery.retry()` — retry only the failed step after a definitive failure. If TransactionStatus could not resolve the original request and the failure remains `INDETERMINATE`, retry checks TransactionStatus again and never resends that request. Earlier successful rebate, point, stored-value load, and tender steps stay committed. There is no SDK retry limit; the register decides when to choose another action.
- `SettlementRecovery.skip()` — skip a definitively failed optional rebate, point, or stored-value charge and continue. Required final tender and stored-value fulfillment steps cannot be skipped, and an indeterminate request cannot be skipped.
- `SettlementRecovery.external(ExternalPayment)` — after the final card tender definitively fails, record a register-managed tender such as cash and continue. Its amount must exactly equal `failure.getAmountDue()`; cashback transactions and indeterminate failures cannot use this substitution. Because TransactionStatus reconciliation happens before `onError`, a recovered card charge can never be silently replaced by the external tender.
- `SettlementRecovery.abandon()` — stop without further TransactionStatus, retry, or unwind. Automatic TransactionStatus reconciliation may already have occurred before `onError`. `onAbandoned` (and `SessionException.getAbandonedSettlement()` for blocking calls) receives the basket, failure, outstanding amount, and committed ledger. The SDK clears this attempt's recovery state but does not consume or clear the basket, so another `settle()` can start immediately. This is an unguarded manual takeover: the register must reconcile the abandoned movements first or risk duplicate discounts and payments.

If the terminal reports `IN_PROGRESS`, the SDK makes up to five TransactionStatus checks, waiting a jittered 800–1200 ms between checks, before reporting an unresolved failure. A register-directed `retry()` then starts another bounded status check; it does not resend the original money-moving request.
