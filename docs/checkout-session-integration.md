...
If the terminal reports `IN_PROGRESS`, the SDK briefly polls TransactionStatus a bounded number of times before reporting an unresolved failure. A register-directed `retry()` then starts another bounded status check; it does not resend the original money-moving request.
